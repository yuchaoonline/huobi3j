<%@ Page Language="C#" Title="" MasterPageFile="~/MMyCorp.master" CodeBehind="Edit.aspx.cs" Inherits="ADeeWu.HuoBi3J.Web.My.Corp.Shop.Products.Edit" %>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#Attributes li span[@attrId!=""]').each(function() {
			...
			optionList = t.attr('optionList'),
			customValue = t.attr('customValue');
            ...
            switch (valueType) {
                ...
                case 1:
                    ...
                    sel.appendTo(t);
                    sel.val(optionList.split(',')[0]);
                    break;
                case 2:
                    break;
                case 3:
                    var optionValues = optionList.split(',');
                    t.find('li').each(function() {
                        var li = $(this),
						value = li.attr('value'),
						text = $.trim(li.html());
                        ...
                        for (var i = 0; i < optionValues.length; i++) {
                            var optionValue = parseInt(optionValues[i]) || 0,
                                chkValue = parseInt(value) || 0;
...
<span class="spl">　</span><a href="/My/Corp/Shop/">在线营销</a><span class="spl">　</span><a href="/My/Corp/Shop/Products/">商品管理</a><span class="spl">　</span><span class="curPos">编辑商品</span>
  </asp:Content>
...
<table border="0" cellspacing="0" cellpadding="0" class="formTable">
  ...
  <tr>
    ...
    <td class="tdRight">
		...
		<li>
			...
			<span class="attrValue" attrName="<%#HttpUtility.HtmlAttributeEncode((string)Eval("Name"))%>" attrId="<%#Eval("id")%>" valueType="<%#Eval("ValueType")%>" isRequire="<%#Eval("IsValueRequire").ToString().ToLower()%>" optionList="<%#HttpUtility.HtmlAttributeEncode(Eval("AttrOptionIDList").ToString())%>" customValue="<%#HttpUtility.HtmlAttributeEncode(Eval("CustomValue").ToString())%>"><%#GetAttrOptions((long)Eval("ID"))%></span>
		</li>
		</ItemTemplate>
		</asp:Repeater>
		</ul>
    ...
    <td class="tdRight">
        <div class="CustomCategory">
            ...
            <div class="masterCategory">
                <input type="checkbox" id="chkCategoryID" runat="server" value='<%#Eval("ID") %>' checked='<%#ADeeWu.HuoBi3J.Libary.Utility.GetLong(Eval("ProductID"),0) == productId%>' /> <%#Eval("Name")%>
                <asp:Repeater ID="rpCPCategories02" runat="server">
                    ...
                    <ItemTemplate>
                    <li><input type="checkbox" id="chkCategoryID" runat="server" value='<%#((System.Data.DataRow)Container.DataItem)["ID"]%>' checked='<%#ADeeWu.HuoBi3J.Libary.Utility.GetLong(((System.Data.DataRow)Container.DataItem)["ProductID"],0) == productId%>' /> <%#((System.Data.DataRow)Container.DataItem)["Name"]%></li>
                    </ItemTemplate>
                </asp:Repeater>
             </div>
            </ItemTemplate>
  ...
  <tr>
    <td class="tdLeft">预订金额：</td>
    <td class="tdRight">
        <asp:RadioButton ID="rbtnPrice1" runat="server" GroupName="rbtnPrice" Checked="true" /> <asp:TextBox ID="txtPrice" runat="server" CssClass="txtNum" Width="60px"></asp:TextBox> 元<asp:RegularExpressionValidator
            ID="revTxtPrice" runat="server"  ControlToValidate="txtPrice" ErrorMessage="请填写正确的价格！" Display="Dynamic" ValidationExpression="^\d+$"></asp:RegularExpressionValidator> 
            <asp:RadioButton ID="rbtnPrice2" runat="server" GroupName="rbtnPrice" /> <asp:TextBox ID="txtReturnPrecent" runat="server" CssClass="txtNum" Width="60px"></asp:TextBox>百分比 <asp:RegularExpressionValidator
            ID="revTxtPrice2" runat="server"  ControlToValidate="txtReturnPrecent" ErrorMessage="请填写正确百分比！" Display="Dynamic" ValidationExpression="^\d+$"></asp:RegularExpressionValidator>
    </td>
  </tr>
  <tr>
    <td class="tdLeft">浮动价格：</td>
    <td class="tdRight">
        <asp:TextBox ID="txtFloatPrice" runat="server" Width="100px"></asp:TextBox>
    </td>
  </tr>
  <tr>
  ...
  <tr >        
        ...
        <td class="tdRight">
            <asp:Button ID="btnSubmit" runat="server" Text="更新"  
