<%@ Page Language="C#" Title="" MasterPageFile="~/MMyCorp.master" CodeBehind="Edit.aspx.cs" Inherits="ADeeWu.HuoBi3J.Web.My.Corp.CorpAgentManage.Roles.Edit" %>
...
<span class="spl">　</span><a href="/My/Corp/CorpAgentManage/Users/">商家代表管理</a><span class="spl">　</span><a href="/My/Corp/CorpAgentManage/Roles/">角色管理</a><span class="spl">　</span><span class="curPos">修改</span>  
  </asp:Content>
...
<Columns>
    ...
    <asp:TemplateField HeaderStyle-CssClass="col_option"  ItemStyle-CssClass="col_option" HeaderText="授权">
        <ItemTemplate>
            <input type="checkbox" name="alowFunctionID" value="<%#Eval("ID") %>" <%#Eval("AuthorizeState").ToString()=="0"?"checked":"" %> onclick="CancelCheckBox($(this).next('[@type=checkbox]'));"/>允许 <input type="checkbox" name="denyFunctionID" value="<%#Eval("ID") %>" onclick="CancelCheckBox($(this).prev('[@type=checkbox]'));" <%#Eval("AuthorizeState").ToString()=="1"?"checked":"" %> />拒绝
        </ItemTemplate>    
    </asp:TemplateField>
</Columns>
</asp:GridView>
