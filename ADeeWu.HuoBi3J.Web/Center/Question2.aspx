<%@ Page Language="C#" Title="" MasterPageFile="~/MIndex.master"  AutoEventWireup="true" CodeBehind="Question2.aspx.cs" Inherits="ADeeWu.HuoBi3J.Web.Center.Question2" %>
...
<script>
    ...
    $(function () {
        ...
        $('.saleman img').ReduceImage();
        $('#title img').ReduceImage();
    })
</script>
...
<asp:Content ID="Content3" ContentPlaceHolderID="main" runat="server">
            ...
            <ItemTemplate>
                <div class="centerP_body">
                    <div class="body_content font14 black70">
                        <p>
                            <span>提问内容：<label class="black32"><%# Eval("Title") %></label></span>
                        ...
                        <p class="mt10">
                            ...
                            <span>添加时间：<label class="black32"><%# Eval("createtime") %></label></span>
                        </p>
                    </div>
                </div>
        ...
        <asp:Repeater ID="rpProduct" runat="server">
            <ItemTemplate>
                <div class="centerP_body">
                    <div class="body_content font14 black70">
                        <p>
                            <span>商品名称：<label class="black32"><%# Eval("Name") %></label></span>
                            <span>商品内容：<label class="black32"><%# Eval("Content") %></label></span>
                            <span>商品分类：<label class="black32"><%# bindCategory(Eval("categoryid")) %></label></span>
                        </p>
                        <p class="mt10">
                            <span>添加时间：<label class="black32"><%# Eval("createtime") %></label></span>
                            <span>商品图片：<a href="<%# Eval("picture") %>" title="<%# Eval("name") %>" class="lb_flowers"><img src="<%# Eval("picture") %>" id="productIMG" alt="商品图片" height="40px" /></a></span>
                        </p>
                    </div>
                </div>
                <div class="centerP_head">
                ...
                <ItemTemplate>
                    <tr height="40px" onmouseover="this.className='jobMenu_hover'" onmouseout="this.className=''" class="">
                        <td class="arc_title">
                            ...
                            <div class="saleman" refid="<%# Eval("uid") %>"><%# Eval("Content") %></div>
                        </td>
                        <td><em class="saleman" refid="<%# Eval("uid") %>"><%# Eval("LoginName") %></em></td>
    ...
    <div id="postdiv" runat="server">
        ...
        <div id="PostQustion">
            ...
            <input type="hidden" name="uid" value="<%= this.UserID %>" />
    ...
    <div id="loginBar">
        <a href="/login.aspx?url=<%= Request.Url %>" id="btnLogin" title="请登录" class="button">
            请登录</a>
    </div>
    <%} %>
