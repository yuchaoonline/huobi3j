...
{	
	...
	public class ProjectHelper : CodeSmith.BaseTemplates.SqlCodeTemplate
	{
		...
		public static string GetCSharpVariableTypeConvertString(string varType,string parseString)
		{
			return GetCSharpVariableTypeConvertString(varType,parseString,false);
		}
		public static string GetCSharpVariableTypeConvertString(string varType,string parseString,bool canNullable)
		{
			switch (varType)
			{				
				case "bool":
				...
				case "ulong":
					return canNullable ? 
					 	string.Format("{1} as {0}",varType,parseString)
						:
						string.Format("{0}.Parse({1}.ToString())",varType,parseString);
						;
				case "object":
				case "string":
				case "Guid":
				default:
					return canNullable ? 
						string.Format("{1} as {0}",varType,parseString)
						:
						string.Format("({0})({1})",varType,parseString)
						;
			}
		}
		...
		/// <param name="cs"></param>
		/// <returns></returns>
		public static bool HasDefaultValue(ColumnSchema cs)
		{
