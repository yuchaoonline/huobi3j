...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Citys{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Citys] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Citys model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@Pid",model.Pid);
		db.Parameters.Append("@Sort",model.Sort.HasValue ? (object)model.Sort.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Citys]([Name],[Pid],[Sort]) values (@Name,@Pid,@Sort);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Citys model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Pid",model.Pid);
		db.Parameters.Append("@Sort",model.Sort.HasValue ? (object)model.Sort.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Citys] set [Name]=@Name,[Pid]=@Pid,[Sort]=@Sort where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Citys",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Citys");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Citys]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Citys");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Citys");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Citys",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Citys GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Citys] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Citys Entity = new ADeeWu.HuoBi3J.Model.Citys();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.Name = (string)(dr["Name"]);
			Entity.Pid = long.Parse(dr["Pid"].ToString());
			Entity.Sort = dr["Sort"] as int?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Citys GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Citys[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Citys GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Citys[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Citys] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Citys[] EntityList = new ADeeWu.HuoBi3J.Model.Citys[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Citys","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Citys","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Citys","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Citys","ID",where,orderBy);
		}
