...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class System_Functions{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [System_Functions] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.System_Functions model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Code",model.Code);
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@FileName",model.FileName);
		db.Parameters.Append("@IsCorpAgentFunc",model.IsCorpAgentFunc);
		db.Parameters.Append("@Description",model.Description);
		    DataTable dt = db.Select("insert into [System_Functions]([Code],[Name],[FileName],[IsCorpAgentFunc],[Description]) values (@Code,@Name,@FileName,@IsCorpAgentFunc,@Description);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.System_Functions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IsCorpAgentFunc",model.IsCorpAgentFunc);
		db.Parameters.Append("@Description",model.Description);
			return db.ExecuteSql("update [System_Functions] set [Code]=@Code,[Name]=@Name,[FileName]=@FileName,[IsCorpAgentFunc]=@IsCorpAgentFunc,[Description]=@Description where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","System_Functions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","System_Functions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [System_Functions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "System_Functions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "System_Functions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "System_Functions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.System_Functions GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [System_Functions] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.System_Functions Entity = new ADeeWu.HuoBi3J.Model.System_Functions();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.Code = (string)(dr["Code"]);
			Entity.Name = (string)(dr["Name"]);
			Entity.FileName = (string)(dr["FileName"]);
			Entity.IsCorpAgentFunc = bool.Parse(dr["IsCorpAgentFunc"].ToString());
			Entity.Description = (string)(dr["Description"]);
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.System_Functions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.System_Functions[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.System_Functions GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.System_Functions[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [System_Functions] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.System_Functions[] EntityList = new ADeeWu.HuoBi3J.Model.System_Functions[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"System_Functions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"System_Functions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"System_Functions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"System_Functions","ID",where,orderBy);
		}
