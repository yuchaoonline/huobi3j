...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_AutoReply{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_AutoReply] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_AutoReply model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Forum_AutoReply]([UserID],[Content],[CreateTime]) values (@UserID,@Content,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_AutoReply model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Content",model.Content );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Forum_AutoReply] set [UserID]=@UserID,[Content]=@Content,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_AutoReply",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_AutoReply");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_AutoReply]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_AutoReply");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_AutoReply");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_AutoReply",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_AutoReply GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_AutoReply] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_AutoReply Entity = new ADeeWu.HuoBi3J.Model.Forum_AutoReply();
		...
		public ADeeWu.HuoBi3J.Model.Forum_AutoReply GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_AutoReply[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_AutoReply GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_AutoReply[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_AutoReply] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_AutoReply[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_AutoReply[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_AutoReply","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AutoReply","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AutoReply","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AutoReply","ID",where,orderBy);
		}
