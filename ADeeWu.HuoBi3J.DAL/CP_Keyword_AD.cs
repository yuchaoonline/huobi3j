...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_Keyword_AD{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_Keyword_AD] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_Keyword_AD model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IsPass",model.IsPass.HasValue ? (object)model.IsPass.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [CP_Keyword_AD]([Name],[Content],[Link],[UserID],[IsPass]) values (@Name,@Content,@Link,@UserID,@IsPass);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_Keyword_AD model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsPass",model.IsPass.HasValue ? (object)model.IsPass.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [CP_Keyword_AD] set [Name]=@Name,[Content]=@Content,[Link]=@Link,[UserID]=@UserID,[IsPass]=@IsPass where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_Keyword_AD",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_Keyword_AD");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_Keyword_AD]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_Keyword_AD");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_Keyword_AD");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_Keyword_AD",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_Keyword_AD] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_Keyword_AD Entity = new ADeeWu.HuoBi3J.Model.CP_Keyword_AD();
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_Keyword_AD[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_Keyword_AD] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_Keyword_AD[] EntityList = new ADeeWu.HuoBi3J.Model.CP_Keyword_AD[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_Keyword_AD","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD","ID",where,orderBy);
		}
