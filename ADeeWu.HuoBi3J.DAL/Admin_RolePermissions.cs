...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_RolePermissions{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_RolePermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CheckState",model.CheckState);
		    DataTable dt = db.Select("insert into [Admin_RolePermissions]([RoleID],[PageID],[CheckState]) values (@RoleID,@PageID,@CheckState);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_RolePermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@PageID",model.PageID.HasValue ? (object)model.PageID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CheckState",model.CheckState);
			return db.ExecuteSql("update [Admin_RolePermissions] set [RoleID]=@RoleID,[PageID]=@PageID,[CheckState]=@CheckState where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_RolePermissions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_RolePermissions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_RolePermissions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_RolePermissions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_RolePermissions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_RolePermissions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.Admin_RolePermissions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_RolePermissions[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"Admin_RolePermissions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_RolePermissions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_RolePermissions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_RolePermissions","ID",where,orderBy);
		}
