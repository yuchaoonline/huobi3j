...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CT_PartnerCorps{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CT_PartnerCorps] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CT_PartnerCorps model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CorpID",model.CorpID);
		db.Parameters.Append("@OfferPercent",model.OfferPercent);
		db.Parameters.Append("@Contract",model.Contract);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CT_PartnerCorps]([CorpID],[OfferPercent],[Contract],[AdminUserID],[CheckState],[CreateTime],[ModifyTime]) values (@CorpID,@OfferPercent,@Contract,@AdminUserID,@CheckState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CT_PartnerCorps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CT_PartnerCorps] set [CorpID]=@CorpID,[OfferPercent]=@OfferPercent,[Contract]=@Contract,[AdminUserID]=@AdminUserID,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CT_PartnerCorps",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CT_PartnerCorps");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CT_PartnerCorps]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CT_PartnerCorps");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CT_PartnerCorps");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CT_PartnerCorps",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CT_PartnerCorps GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CT_PartnerCorps] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CT_PartnerCorps Entity = new ADeeWu.HuoBi3J.Model.CT_PartnerCorps();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CorpID = long.Parse(dr["CorpID"].ToString());
			Entity.OfferPercent = int.Parse(dr["OfferPercent"].ToString());
			Entity.Contract = (string)(dr["Contract"]);
		...
		public ADeeWu.HuoBi3J.Model.CT_PartnerCorps GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CT_PartnerCorps[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CT_PartnerCorps GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CT_PartnerCorps[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CT_PartnerCorps] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CT_PartnerCorps[] EntityList = new ADeeWu.HuoBi3J.Model.CT_PartnerCorps[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CT_PartnerCorps","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_PartnerCorps","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_PartnerCorps","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_PartnerCorps","ID",where,orderBy);
		}
