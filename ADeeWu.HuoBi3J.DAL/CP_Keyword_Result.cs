...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_Keyword_Result{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_Keyword_Result] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_Keyword_Result model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Link",model.Link  );
		    DataTable dt = db.Select("insert into [CP_Keyword_Result]([KeywordID],[Title],[Content],[Link]) values (@KeywordID,@Title,@Content,@Link);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_Keyword_Result model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Content",model.Content  );
		db.Parameters.Append("@Link",model.Link  );
			return db.ExecuteSql("update [CP_Keyword_Result] set [KeywordID]=@KeywordID,[Title]=@Title,[Content]=@Content,[Link]=@Link where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_Keyword_Result",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_Keyword_Result");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_Keyword_Result]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_Keyword_Result");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_Keyword_Result");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_Keyword_Result",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Result GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_Keyword_Result] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_Keyword_Result Entity = new ADeeWu.HuoBi3J.Model.CP_Keyword_Result();
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Result GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_Keyword_Result[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Result GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Result[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_Keyword_Result] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_Keyword_Result[] EntityList = new ADeeWu.HuoBi3J.Model.CP_Keyword_Result[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_Keyword_Result","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Result","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Result","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Result","ID",where,orderBy);
		}
