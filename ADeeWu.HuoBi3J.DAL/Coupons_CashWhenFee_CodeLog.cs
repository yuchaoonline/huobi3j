...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Coupons_CashWhenFee_CodeLog{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Coupons_CashWhenFee_CodeLog] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UseCode",model.UseCode );
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Coupons_CashWhenFee_CodeLog]([UseCode],[CodeID],[UseCount],[CreateTime]) values (@UseCode,@CodeID,@UseCount,@CreateTime);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@UseCount",model.UseCount.HasValue ? (object)model.UseCount.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Coupons_CashWhenFee_CodeLog] set [UseCode]=@UseCode,[CodeID]=@CodeID,[UseCount]=@UseCount,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Coupons_CashWhenFee_CodeLog",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Coupons_CashWhenFee_CodeLog");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Coupons_CashWhenFee_CodeLog]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Coupons_CashWhenFee_CodeLog");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Coupons_CashWhenFee_CodeLog");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Coupons_CashWhenFee_CodeLog",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Coupons_CashWhenFee_CodeLog] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog Entity = new ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.UseCode = dr["UseCode"] as string;
		...
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Coupons_CashWhenFee_CodeLog] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog[] EntityList = new ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_CodeLog[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Coupons_CashWhenFee_CodeLog","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_CodeLog","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_CodeLog","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_CodeLog","ID",where,orderBy);
		}
