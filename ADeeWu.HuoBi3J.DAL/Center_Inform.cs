...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_Inform{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_Inform] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_Inform model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ContentID",model.ContentID.HasValue ? (object)model.ContentID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@InformType",model.InformType.HasValue ? (object)model.InformType.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Center_Inform]([ContentID],[CreateTime],[InformType]) values (@ContentID,@CreateTime,@InformType);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_Inform model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@InformType",model.InformType.HasValue ? (object)model.InformType.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Center_Inform] set [ContentID]=@ContentID,[CreateTime]=@CreateTime,[InformType]=@InformType where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_Inform",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_Inform");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_Inform]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_Inform");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_Inform");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_Inform",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_Inform GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_Inform] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_Inform Entity = new ADeeWu.HuoBi3J.Model.Center_Inform();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.ContentID = dr["ContentID"] as int?;
			Entity.CreateTime = dr["CreateTime"] as DateTime?;
			Entity.InformType = dr["InformType"] as int?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Center_Inform GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_Inform[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_Inform GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_Inform[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_Inform] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_Inform[] EntityList = new ADeeWu.HuoBi3J.Model.Center_Inform[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_Inform","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Inform","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Inform","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Inform","ID",where,orderBy);
		}
