...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Market_SeekerFavs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Market_SeekerFavs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Market_SeekerFavs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@MarketInfoID",model.MarketInfoID);
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [Market_SeekerFavs]([MarketInfoID],[UserID],[Notes],[CreateTime]) values (@MarketInfoID,@UserID,@Notes,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Market_SeekerFavs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@MarketInfoID",model.MarketInfoID);
		...
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [Market_SeekerFavs] set [MarketInfoID]=@MarketInfoID,[UserID]=@UserID,[Notes]=@Notes,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Market_SeekerFavs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] set 1=1","Market_SeekerFavs");
			if(!string.IsNullOrEmpty(where)){
				sql += " where " + where;
			}
			db.Parameters.Clear();
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Market_SeekerFavs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Market_SeekerFavs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Market_SeekerFavs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Market_SeekerFavs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Market_SeekerFavs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Market_SeekerFavs] where 1=1  and [ID]=@ID");
			...
			Entity.MarketInfoID = long.Parse(dr["MarketInfoID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.Notes = (string)(dr["Notes"].ToString());
			Entity.CreateTime = DateTime.Parse(dr["CreateTime"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Market_SeekerFavs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Market_SeekerFavs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Market_SeekerFavs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Market_SeekerFavs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Market_SeekerFavs[] EntityList = new ADeeWu.HuoBi3J.Model.Market_SeekerFavs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Market_SeekerFavs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_SeekerFavs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_SeekerFavs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_SeekerFavs","ID",where,orderBy);
		}
