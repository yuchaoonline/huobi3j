...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_OrderDetails{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_OrderDetails] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_OrderDetails model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@OrderID",model.OrderID);
		...
		db.Parameters.Append("@ProductID",model.ProductID);
		db.Parameters.Append("@ProductName",model.ProductName);
		db.Parameters.Append("@NowPrice",model.NowPrice);
		db.Parameters.Append("@Quantity",model.Quantity);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_OrderDetails]([OrderID],[SellerCorpID],[SellerUserID],[BuyerUserID],[ProductID],[ProductName],[NowPrice],[Quantity],[Note],[CreateTime],[ModifyTime]) values (@OrderID,@SellerCorpID,@SellerUserID,@BuyerUserID,@ProductID,@ProductName,@NowPrice,@Quantity,@Note,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_OrderDetails model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_OrderDetails] set [OrderID]=@OrderID,[SellerCorpID]=@SellerCorpID,[SellerUserID]=@SellerUserID,[BuyerUserID]=@BuyerUserID,[ProductID]=@ProductID,[ProductName]=@ProductName,[NowPrice]=@NowPrice,[Quantity]=@Quantity,[Note]=@Note,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_OrderDetails",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_OrderDetails");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_OrderDetails]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_OrderDetails");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_OrderDetails");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_OrderDetails",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_OrderDetails GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_OrderDetails] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_OrderDetails Entity = new ADeeWu.HuoBi3J.Model.Shop_OrderDetails();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.OrderID = long.Parse(dr["OrderID"].ToString());
			...
			Entity.ProductID = long.Parse(dr["ProductID"].ToString());
			Entity.ProductName = (string)(dr["ProductName"]);
			Entity.NowPrice = decimal.Parse(dr["NowPrice"].ToString());
			Entity.Quantity = int.Parse(dr["Quantity"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Shop_OrderDetails GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_OrderDetails[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_OrderDetails GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_OrderDetails[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_OrderDetails] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_OrderDetails[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_OrderDetails[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_OrderDetails","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_OrderDetails","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_OrderDetails","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_OrderDetails","ID",where,orderBy);
		}
