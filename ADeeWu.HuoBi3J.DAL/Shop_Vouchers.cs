...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_Vouchers{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_Vouchers] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_Vouchers model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_Vouchers]([SellerCorpID],[SellerUserID],[BuyerUserID],[Title],[Content],[CreateTime],[ModifyTime]) values (@SellerCorpID,@SellerUserID,@BuyerUserID,@Title,@Content,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_Vouchers model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_Vouchers] set [SellerCorpID]=@SellerCorpID,[SellerUserID]=@SellerUserID,[BuyerUserID]=@BuyerUserID,[Title]=@Title,[Content]=@Content,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_Vouchers",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_Vouchers");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_Vouchers]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_Vouchers");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_Vouchers");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_Vouchers",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_Vouchers GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_Vouchers] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_Vouchers Entity = new ADeeWu.HuoBi3J.Model.Shop_Vouchers();
		...
		public ADeeWu.HuoBi3J.Model.Shop_Vouchers GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_Vouchers[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_Vouchers GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_Vouchers[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_Vouchers] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_Vouchers[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_Vouchers[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_Vouchers","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Vouchers","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Vouchers","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Vouchers","ID",where,orderBy);
		}
