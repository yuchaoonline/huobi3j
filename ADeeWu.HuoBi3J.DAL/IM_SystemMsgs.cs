...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_SystemMsgs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_SystemMsgs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_SystemMsgs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@MessageName",model.MessageName);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_SystemMsgs]([Title],[Content],[ToUIN],[ToUserID],[MessageName],[CreateTime]) values (@Title,@Content,@ToUIN,@ToUserID,@MessageName,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_SystemMsgs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@MessageName",model.MessageName);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_SystemMsgs] set [Title]=@Title,[Content]=@Content,[ToUIN]=@ToUIN,[ToUserID]=@ToUserID,[MessageName]=@MessageName,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_SystemMsgs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_SystemMsgs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_SystemMsgs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_SystemMsgs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_SystemMsgs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_SystemMsgs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_SystemMsgs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_SystemMsgs] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_SystemMsgs Entity = new ADeeWu.HuoBi3J.Model.IM_SystemMsgs();
			...
			Entity.MessageName = (string)(dr["MessageName"]);
			Entity.CreateTime = DateTime.Parse(dr["CreateTime"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.IM_SystemMsgs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_SystemMsgs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_SystemMsgs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_SystemMsgs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_SystemMsgs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_SystemMsgs[] EntityList = new ADeeWu.HuoBi3J.Model.IM_SystemMsgs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_SystemMsgs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_SystemMsgs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_SystemMsgs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_SystemMsgs","ID",where,orderBy);
		}
