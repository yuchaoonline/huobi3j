...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CorpAgents{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CorpAgents] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CorpAgents model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@AgentCorpID",model.AgentCorpID);
		db.Parameters.Append("@RoleID",model.RoleID);
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@ManageProvinceID",model.ManageProvinceID);
		db.Parameters.Append("@ManageCityID",model.ManageCityID);
		db.Parameters.Append("@ManageAreaID",model.ManageAreaID);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CorpAgents]([AgentCorpID],[RoleID],[UserID],[ManageProvinceID],[ManageCityID],[ManageAreaID],[CheckState],[CreateTime],[ModifyTime]) values (@AgentCorpID,@RoleID,@UserID,@ManageProvinceID,@ManageCityID,@ManageAreaID,@CheckState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CorpAgents model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CorpAgents] set [AgentCorpID]=@AgentCorpID,[RoleID]=@RoleID,[UserID]=@UserID,[ManageProvinceID]=@ManageProvinceID,[ManageCityID]=@ManageCityID,[ManageAreaID]=@ManageAreaID,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CorpAgents",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CorpAgents");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CorpAgents]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CorpAgents");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CorpAgents");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CorpAgents",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CorpAgents GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CorpAgents] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CorpAgents Entity = new ADeeWu.HuoBi3J.Model.CorpAgents();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.AgentCorpID = long.Parse(dr["AgentCorpID"].ToString());
			Entity.RoleID = long.Parse(dr["RoleID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.ManageProvinceID = long.Parse(dr["ManageProvinceID"].ToString());
			Entity.ManageCityID = long.Parse(dr["ManageCityID"].ToString());
			Entity.ManageAreaID = long.Parse(dr["ManageAreaID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CorpAgents GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CorpAgents[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CorpAgents GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CorpAgents[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CorpAgents] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CorpAgents[] EntityList = new ADeeWu.HuoBi3J.Model.CorpAgents[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CorpAgents","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CorpAgents","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CorpAgents","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CorpAgents","ID",where,orderBy);
		}
