...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Corporations{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Corporations] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Corporations model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@CorpName",model.CorpName);
		db.Parameters.Append("@Intro",model.Intro);
		db.Parameters.Append("@LinkMan",model.LinkMan);
		db.Parameters.Append("@Tel",model.Tel);
		db.Parameters.Append("@Property",model.Property);
		db.Parameters.Append("@Employee",model.Employee);
		db.Parameters.Append("@CallingID",model.CallingID);
		db.Parameters.Append("@Calling",model.Calling);
		...
		db.Parameters.Append("@Service",model.Service);
		...
		db.Parameters.Append("@AdminUserID",model.AdminUserID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Corporations]([UserID],[CorpName],[Intro],[LinkMan],[Tel],[Property],[Employee],[CallingID],[Calling],[ProvinceID],[CityID],[AreaID],[Province],[City],[Area],[Address],[Service],[Lat],[Lng],[CheckState],[VisitCount],[AdminUserID],[CreateTime],[ModifyTime]) values (@UserID,@CorpName,@Intro,@LinkMan,@Tel,@Property,@Employee,@CallingID,@Calling,@ProvinceID,@CityID,@AreaID,@Province,@City,@Area,@Address,@Service,@Lat,@Lng,@CheckState,@VisitCount,@AdminUserID,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Corporations model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Corporations] set [UserID]=@UserID,[CorpName]=@CorpName,[Intro]=@Intro,[LinkMan]=@LinkMan,[Tel]=@Tel,[Property]=@Property,[Employee]=@Employee,[CallingID]=@CallingID,[Calling]=@Calling,[ProvinceID]=@ProvinceID,[CityID]=@CityID,[AreaID]=@AreaID,[Province]=@Province,[City]=@City,[Area]=@Area,[Address]=@Address,[Service]=@Service,[Lat]=@Lat,[Lng]=@Lng,[CheckState]=@CheckState,[VisitCount]=@VisitCount,[AdminUserID]=@AdminUserID,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Corporations",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Corporations");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Corporations]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Corporations");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Corporations");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Corporations",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Corporations GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Corporations] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Corporations Entity = new ADeeWu.HuoBi3J.Model.Corporations();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.CorpName = (string)(dr["CorpName"]);
			Entity.Intro = (string)(dr["Intro"]);
			Entity.LinkMan = (string)(dr["LinkMan"]);
			Entity.Tel = (string)(dr["Tel"]);
			Entity.Property = (string)(dr["Property"]);
			Entity.Employee = (string)(dr["Employee"]);
			Entity.CallingID = long.Parse(dr["CallingID"].ToString());
			Entity.Calling = (string)(dr["Calling"]);
			...
			Entity.Service = (string)(dr["Service"]);
			Entity.Lat = decimal.Parse(dr["Lat"].ToString());
			Entity.Lng = decimal.Parse(dr["Lng"].ToString());
			Entity.CheckState = int.Parse(dr["CheckState"].ToString());
			Entity.VisitCount = int.Parse(dr["VisitCount"].ToString());
			Entity.AdminUserID = long.Parse(dr["AdminUserID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Corporations GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Corporations[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Corporations GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Corporations[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Corporations] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Corporations[] EntityList = new ADeeWu.HuoBi3J.Model.Corporations[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Corporations","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Corporations","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Corporations","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Corporations","ID",where,orderBy);
		}
