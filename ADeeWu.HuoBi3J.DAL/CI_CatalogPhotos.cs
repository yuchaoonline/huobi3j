...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CI_CatalogPhotos{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CI_CatalogPhotos] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CI_CatalogPhotos model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CatalogsID",model.CatalogsID);
		...
		db.Parameters.Append("@URL",model.URL);
		    DataTable dt = db.Select("insert into [CI_CatalogPhotos]([CatalogsID],[CorpID],[Title],[Summary],[Content],[Sequence],[IsRecommend],[IsHidden],[VisitCount],[CheckState],[CreateTime],[ModifyTime],[URL]) values (@CatalogsID,@CorpID,@Title,@Summary,@Content,@Sequence,@IsRecommend,@IsHidden,@VisitCount,@CheckState,@CreateTime,@ModifyTime,@URL);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CI_CatalogPhotos model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		db.Parameters.Append("@URL",model.URL);
			return db.ExecuteSql("update [CI_CatalogPhotos] set [CatalogsID]=@CatalogsID,[CorpID]=@CorpID,[Title]=@Title,[Summary]=@Summary,[Content]=@Content,[Sequence]=@Sequence,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[VisitCount]=@VisitCount,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime,[URL]=@URL where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CI_CatalogPhotos",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CI_CatalogPhotos");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CI_CatalogPhotos]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CI_CatalogPhotos");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CI_CatalogPhotos");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CI_CatalogPhotos",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CI_CatalogPhotos GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CI_CatalogPhotos] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CI_CatalogPhotos Entity = new ADeeWu.HuoBi3J.Model.CI_CatalogPhotos();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CatalogsID = long.Parse(dr["CatalogsID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CI_CatalogPhotos GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CI_CatalogPhotos[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CI_CatalogPhotos GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CI_CatalogPhotos[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CI_CatalogPhotos] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CI_CatalogPhotos[] EntityList = new ADeeWu.HuoBi3J.Model.CI_CatalogPhotos[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CI_CatalogPhotos","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_CatalogPhotos","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_CatalogPhotos","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_CatalogPhotos","ID",where,orderBy);
		}
