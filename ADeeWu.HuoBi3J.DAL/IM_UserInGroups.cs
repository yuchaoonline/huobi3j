...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_UserInGroups{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_UserInGroups] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_UserInGroups model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@MemberUIN",model.MemberUIN);
		db.Parameters.Append("@MemberUserID",model.MemberUserID);
		db.Parameters.Append("@GroupUIN",model.GroupUIN);
		db.Parameters.Append("@GroupID",model.GroupID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_UserInGroups]([MemberUIN],[MemberUserID],[GroupUIN],[GroupID],[CreateTime]) values (@MemberUIN,@MemberUserID,@GroupUIN,@GroupID,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_UserInGroups model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@GroupID",model.GroupID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_UserInGroups] set [MemberUIN]=@MemberUIN,[MemberUserID]=@MemberUserID,[GroupUIN]=@GroupUIN,[GroupID]=@GroupID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_UserInGroups",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_UserInGroups");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_UserInGroups]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_UserInGroups");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_UserInGroups");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_UserInGroups",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_UserInGroups GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_UserInGroups] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_UserInGroups Entity = new ADeeWu.HuoBi3J.Model.IM_UserInGroups();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.MemberUIN = (string)(dr["MemberUIN"]);
			Entity.MemberUserID = long.Parse(dr["MemberUserID"].ToString());
			Entity.GroupUIN = (string)(dr["GroupUIN"]);
			Entity.GroupID = long.Parse(dr["GroupID"].ToString());
			Entity.CreateTime = DateTime.Parse(dr["CreateTime"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.IM_UserInGroups GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_UserInGroups[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_UserInGroups GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_UserInGroups[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_UserInGroups] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_UserInGroups[] EntityList = new ADeeWu.HuoBi3J.Model.IM_UserInGroups[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_UserInGroups","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UserInGroups","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UserInGroups","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UserInGroups","ID",where,orderBy);
		}
