...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_QuaAgentInCorps{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_QuaAgentInCorps] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CA_QuaAgentInCorps]([UserID],[CorpID],[CheckState],[CreateTime],[ModifyTime]) values (@UserID,@CorpID,@CheckState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CA_QuaAgentInCorps] set [UserID]=@UserID,[CorpID]=@CorpID,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_QuaAgentInCorps",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_QuaAgentInCorps");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_QuaAgentInCorps]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_QuaAgentInCorps");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_QuaAgentInCorps");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_QuaAgentInCorps",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_QuaAgentInCorps] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps Entity = new ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps();
		...
		public ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_QuaAgentInCorps] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps[] EntityList = new ADeeWu.HuoBi3J.Model.CA_QuaAgentInCorps[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_QuaAgentInCorps","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QuaAgentInCorps","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QuaAgentInCorps","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QuaAgentInCorps","ID",where,orderBy);
		}
