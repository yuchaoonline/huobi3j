...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_CoinLog{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_CoinLog] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_CoinLog model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CustomerUserID",model.CustomerUserID.HasValue ? (object)model.CustomerUserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@AgentUserID",model.AgentUserID.HasValue ? (object)model.AgentUserID.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Forum_CoinLog]([CustomerUserID],[AgentUserID],[Score],[CreateTime]) values (@CustomerUserID,@AgentUserID,@Score,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_CoinLog model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Score",model.Score.HasValue ? (object)model.Score.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Forum_CoinLog] set [CustomerUserID]=@CustomerUserID,[AgentUserID]=@AgentUserID,[Score]=@Score,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_CoinLog",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_CoinLog");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_CoinLog]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_CoinLog");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_CoinLog");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_CoinLog",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_CoinLog GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_CoinLog] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_CoinLog Entity = new ADeeWu.HuoBi3J.Model.Forum_CoinLog();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CustomerUserID = dr["CustomerUserID"] as long?;
			Entity.AgentUserID = dr["AgentUserID"] as long?;
		...
		public ADeeWu.HuoBi3J.Model.Forum_CoinLog GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_CoinLog[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_CoinLog GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_CoinLog[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_CoinLog] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_CoinLog[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_CoinLog[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_CoinLog","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_CoinLog","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_CoinLog","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_CoinLog","ID",where,orderBy);
		}
