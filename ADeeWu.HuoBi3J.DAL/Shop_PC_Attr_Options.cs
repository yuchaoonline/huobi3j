...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_PC_Attr_Options{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_PC_Attr_Options] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@AttrID",model.AttrID);
		db.Parameters.Append("@Value",model.Value);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_PC_Attr_Options]([AttrID],[Value],[ItemCount],[Sequence],[CreateTime],[ModifyTime]) values (@AttrID,@Value,@ItemCount,@Sequence,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_PC_Attr_Options] set [AttrID]=@AttrID,[Value]=@Value,[ItemCount]=@ItemCount,[Sequence]=@Sequence,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_PC_Attr_Options",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_PC_Attr_Options");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_PC_Attr_Options]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_PC_Attr_Options");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_PC_Attr_Options");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_PC_Attr_Options",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_PC_Attr_Options] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options Entity = new ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.AttrID = long.Parse(dr["AttrID"].ToString());
			Entity.Value = (string)(dr["Value"]);
		...
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_PC_Attr_Options] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_PC_Attr_Options[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_PC_Attr_Options","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attr_Options","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attr_Options","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attr_Options","ID",where,orderBy);
		}
