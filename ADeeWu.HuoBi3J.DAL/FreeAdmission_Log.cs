...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class FreeAdmission_Log{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [FreeAdmission_Log] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.FreeAdmission_Log model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@FreeAdmissionID",model.FreeAdmissionID.HasValue ? (object)model.FreeAdmissionID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [FreeAdmission_Log]([UserID],[FreeAdmissionID],[CreateTime]) values (@UserID,@FreeAdmissionID,@CreateTime);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.FreeAdmission_Log model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@FreeAdmissionID",model.FreeAdmissionID.HasValue ? (object)model.FreeAdmissionID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [FreeAdmission_Log] set [UserID]=@UserID,[FreeAdmissionID]=@FreeAdmissionID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","FreeAdmission_Log",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","FreeAdmission_Log");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [FreeAdmission_Log]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "FreeAdmission_Log");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "FreeAdmission_Log");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "FreeAdmission_Log",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission_Log GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [FreeAdmission_Log] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.FreeAdmission_Log Entity = new ADeeWu.HuoBi3J.Model.FreeAdmission_Log();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.UserID = dr["UserID"] as int?;
			Entity.FreeAdmissionID = dr["FreeAdmissionID"] as int?;
			Entity.CreateTime = DateTime.Parse(dr["CreateTime"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission_Log GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.FreeAdmission_Log[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission_Log GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.FreeAdmission_Log[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [FreeAdmission_Log] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.FreeAdmission_Log[] EntityList = new ADeeWu.HuoBi3J.Model.FreeAdmission_Log[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"FreeAdmission_Log","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission_Log","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission_Log","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission_Log","ID",where,orderBy);
		}
