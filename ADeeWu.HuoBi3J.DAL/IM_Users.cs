...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_Users{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_Users model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@NickName",model.NickName);
		db.Parameters.Append("@HeadPic",model.HeadPic);
		db.Parameters.Append("@Sex",model.Sex);
		db.Parameters.Append("@SignText",model.SignText);
		db.Parameters.Append("@Birthday",model.Birthday.HasValue ? (object)model.Birthday.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@Profession",model.Profession);
		db.Parameters.Append("@School",model.School);
		db.Parameters.Append("@HomePage",model.HomePage);
		db.Parameters.Append("@Introduce",model.Introduce);
		db.Parameters.Append("@CheckState",model.CheckState);
		db.Parameters.Append("@LastLoginTime",model.LastLoginTime);
		db.Parameters.Append("@LastLoginIP",model.LastLoginIP);
		db.Parameters.Append("@LoginTimes",model.LoginTimes);
		db.Parameters.Append("@LoginState",model.LoginState);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [IM_Users]([UIN],[UserID],[NickName],[HeadPic],[Sex],[SignText],[Birthday],[Tel],[Email],[ProvinceID],[CityID],[AreaID],[Province],[City],[Area],[Profession],[School],[HomePage],[Introduce],[CheckState],[LastLoginTime],[LastLoginIP],[LoginTimes],[LoginState],[CreateTime],[ModifyTime]) values (@UIN,@UserID,@NickName,@HeadPic,@Sex,@SignText,@Birthday,@Tel,@Email,@ProvinceID,@CityID,@AreaID,@Province,@City,@Area,@Profession,@School,@HomePage,@Introduce,@CheckState,@LastLoginTime,@LastLoginIP,@LoginTimes,@LoginState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_Users model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [IM_Users] set [UserID]=@UserID,[NickName]=@NickName,[HeadPic]=@HeadPic,[Sex]=@Sex,[SignText]=@SignText,[Birthday]=@Birthday,[Tel]=@Tel,[Email]=@Email,[ProvinceID]=@ProvinceID,[CityID]=@CityID,[AreaID]=@AreaID,[Province]=@Province,[City]=@City,[Area]=@Area,[Profession]=@Profession,[School]=@School,[HomePage]=@HomePage,[Introduce]=@Introduce,[CheckState]=@CheckState,[LastLoginTime]=@LastLoginTime,[LastLoginIP]=@LastLoginIP,[LoginTimes]=@LoginTimes,[LoginState]=@LoginState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [UIN]=@UIN");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_Users",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_Users");
		...
		public int Delete(long  ID,string  UIN)
		{
			db.Parameters.Clear();
			db.Parameters.Append("@ID",ID);
			db.Parameters.Append("@UIN",UIN);
			return db.ExecuteSql("delete from [IM_Users]  where  @ID=ID and @UIN=UIN");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_Users");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_Users");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_Users",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.IM_Users GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_Users] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_Users Entity = new ADeeWu.HuoBi3J.Model.IM_Users();
			Entity.ID = long.Parse(dr["ID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.IM_Users GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_Users[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"IM_Users","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Users","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Users","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Users","ID",where,orderBy);
		}
