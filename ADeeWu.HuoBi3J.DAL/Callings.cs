...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Callings{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Callings] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Callings model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Sequence",model.Sequence.HasValue ? (object)model.Sequence.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Callings]([ParentID],[Name],[ItemCount],[ParentPath],[Depth],[IsRecommend],[IsHidden],[Sequence],[CreateTime],[ModifyTime]) values (@ParentID,@Name,@ItemCount,@ParentPath,@Depth,@IsRecommend,@IsHidden,@Sequence,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Callings model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Callings] set [ParentID]=@ParentID,[Name]=@Name,[ItemCount]=@ItemCount,[ParentPath]=@ParentPath,[Depth]=@Depth,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[Sequence]=@Sequence,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Callings",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Callings");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Callings]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Callings");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Callings");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Callings",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Callings GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Callings] where 1=1  and [ID]=@ID");
			...
			Entity.Sequence = dr["Sequence"] as int?;
		...
		public ADeeWu.HuoBi3J.Model.Callings GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Callings[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Callings GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Callings[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Callings] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Callings[] EntityList = new ADeeWu.HuoBi3J.Model.Callings[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Callings","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Callings","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Callings","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Callings","ID",where,orderBy);
		}
