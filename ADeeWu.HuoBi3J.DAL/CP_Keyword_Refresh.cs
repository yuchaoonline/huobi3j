...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_Keyword_Refresh{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_Keyword_Refresh] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@LastTime",model.LastTime.HasValue ? (object)model.LastTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [CP_Keyword_Refresh]([KeywordID],[LastTime]) values (@KeywordID,@LastTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@KeywordID",model.KeywordID.HasValue ? (object)model.KeywordID.Value : (object)DBNull.Value );
		db.Parameters.Append("@LastTime",model.LastTime.HasValue ? (object)model.LastTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [CP_Keyword_Refresh] set [KeywordID]=@KeywordID,[LastTime]=@LastTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_Keyword_Refresh",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_Keyword_Refresh");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_Keyword_Refresh]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_Keyword_Refresh");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_Keyword_Refresh");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_Keyword_Refresh",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_Keyword_Refresh] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh Entity = new ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh();
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_Keyword_Refresh] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh[] EntityList = new ADeeWu.HuoBi3J.Model.CP_Keyword_Refresh[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_Keyword_Refresh","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Refresh","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Refresh","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_Refresh","ID",where,orderBy);
		}
