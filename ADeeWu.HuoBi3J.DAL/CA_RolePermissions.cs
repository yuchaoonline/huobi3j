...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_RolePermissions{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_RolePermissions] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_RolePermissions model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ManagerCorpID",model.ManagerCorpID);
		db.Parameters.Append("@RoleID",model.RoleID.HasValue ? (object)model.RoleID.Value : (object)DBNull.Value );
		db.Parameters.Append("@FunctionID",model.FunctionID.HasValue ? (object)model.FunctionID.Value : (object)DBNull.Value );
		db.Parameters.Append("@AuthorizeState",model.AuthorizeState);
		    DataTable dt = db.Select("insert into [CA_RolePermissions]([ManagerCorpID],[RoleID],[FunctionID],[AuthorizeState]) values (@ManagerCorpID,@RoleID,@FunctionID,@AuthorizeState);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_RolePermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@FunctionID",model.FunctionID.HasValue ? (object)model.FunctionID.Value : (object)DBNull.Value );
		db.Parameters.Append("@AuthorizeState",model.AuthorizeState);
			return db.ExecuteSql("update [CA_RolePermissions] set [ManagerCorpID]=@ManagerCorpID,[RoleID]=@RoleID,[FunctionID]=@FunctionID,[AuthorizeState]=@AuthorizeState where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_RolePermissions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_RolePermissions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_RolePermissions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_RolePermissions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_RolePermissions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_RolePermissions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_RolePermissions GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_RolePermissions] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_RolePermissions Entity = new ADeeWu.HuoBi3J.Model.CA_RolePermissions();
			...
			Entity.FunctionID = dr["FunctionID"] as long?;
			Entity.AuthorizeState = int.Parse(dr["AuthorizeState"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.CA_RolePermissions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_RolePermissions[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_RolePermissions GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_RolePermissions[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_RolePermissions] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_RolePermissions[] EntityList = new ADeeWu.HuoBi3J.Model.CA_RolePermissions[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_RolePermissions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_RolePermissions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_RolePermissions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_RolePermissions","ID",where,orderBy);
		}
