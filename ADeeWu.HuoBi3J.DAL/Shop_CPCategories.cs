...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_CPCategories{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_CPCategories] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_CPCategories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ImgUrl",model.ImgUrl);
		db.Parameters.Append("@ShowImg",model.ShowImg);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_CPCategories]([ShopID],[Name],[ItemCount],[ParentID],[Depth],[ImgUrl],[ShowImg],[Sequence],[IsHidden],[CreateTime],[ModifyTime]) values (@ShopID,@Name,@ItemCount,@ParentID,@Depth,@ImgUrl,@ShowImg,@Sequence,@IsHidden,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_CPCategories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_CPCategories] set [ShopID]=@ShopID,[Name]=@Name,[ItemCount]=@ItemCount,[ParentID]=@ParentID,[Depth]=@Depth,[ImgUrl]=@ImgUrl,[ShowImg]=@ShowImg,[Sequence]=@Sequence,[IsHidden]=@IsHidden,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_CPCategories",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_CPCategories");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_CPCategories]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_CPCategories");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_CPCategories");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_CPCategories",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_CPCategories GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_CPCategories] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_CPCategories Entity = new ADeeWu.HuoBi3J.Model.Shop_CPCategories();
			...
			Entity.ImgUrl = (string)(dr["ImgUrl"]);
			Entity.ShowImg = bool.Parse(dr["ShowImg"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Shop_CPCategories GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_CPCategories[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_CPCategories GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_CPCategories[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_CPCategories] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_CPCategories[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_CPCategories[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_CPCategories","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_CPCategories","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_CPCategories","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_CPCategories","ID",where,orderBy);
		}
