...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_FriendInTeams{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_FriendInTeams] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_FriendInTeams model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@MeUserID",model.MeUserID);
		db.Parameters.Append("@MeUIN",model.MeUIN);
		db.Parameters.Append("@FriendUserID",model.FriendUserID);
		db.Parameters.Append("@FriendUIN",model.FriendUIN);
		db.Parameters.Append("@FriendTeamID",model.FriendTeamID);
		    DataTable dt = db.Select("insert into [IM_FriendInTeams]([MeUserID],[MeUIN],[FriendUserID],[FriendUIN],[FriendTeamID]) values (@MeUserID,@MeUIN,@FriendUserID,@FriendUIN,@FriendTeamID);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_FriendInTeams model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@FriendUIN",model.FriendUIN);
		db.Parameters.Append("@FriendTeamID",model.FriendTeamID);
			return db.ExecuteSql("update [IM_FriendInTeams] set [MeUserID]=@MeUserID,[MeUIN]=@MeUIN,[FriendUserID]=@FriendUserID,[FriendUIN]=@FriendUIN,[FriendTeamID]=@FriendTeamID where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_FriendInTeams",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_FriendInTeams");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_FriendInTeams]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_FriendInTeams");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_FriendInTeams");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_FriendInTeams",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_FriendInTeams GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_FriendInTeams] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_FriendInTeams Entity = new ADeeWu.HuoBi3J.Model.IM_FriendInTeams();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.MeUserID = long.Parse(dr["MeUserID"].ToString());
			Entity.MeUIN = (string)(dr["MeUIN"]);
			Entity.FriendUserID = long.Parse(dr["FriendUserID"].ToString());
			Entity.FriendUIN = (string)(dr["FriendUIN"]);
			Entity.FriendTeamID = long.Parse(dr["FriendTeamID"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.IM_FriendInTeams GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_FriendInTeams[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_FriendInTeams GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_FriendInTeams[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_FriendInTeams] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_FriendInTeams[] EntityList = new ADeeWu.HuoBi3J.Model.IM_FriendInTeams[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_FriendInTeams","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendInTeams","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendInTeams","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendInTeams","ID",where,orderBy);
		}
