...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class tblHistory{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [tblHistory] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.tblHistory model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@FromUIN",model.FromUIN);
		db.Parameters.Append("@ToUIN",model.ToUIN);
		db.Parameters.Append("@nTime",model.nTime);
		db.Parameters.Append("@sMsg",model.sMsg);
		db.Parameters.Append("@nType",model.nType);
		db.Parameters.Append("@GroupChatID",model.GroupChatID);
		db.Parameters.Append("@sIP",model.sIP);
		    DataTable dt = db.Select("insert into [tblHistory]([FromUIN],[ToUIN],[nTime],[sMsg],[nType],[GroupChatID],[sIP]) values (@FromUIN,@ToUIN,@nTime,@sMsg,@nType,@GroupChatID,@sIP);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.tblHistory model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@GroupChatID",model.GroupChatID);
		db.Parameters.Append("@sIP",model.sIP);
			return db.ExecuteSql("update [tblHistory] set [FromUIN]=@FromUIN,[ToUIN]=@ToUIN,[nTime]=@nTime,[sMsg]=@sMsg,[nType]=@nType,[GroupChatID]=@GroupChatID,[sIP]=@sIP where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","tblHistory",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] set 1=1","tblHistory");
			if(!string.IsNullOrEmpty(where)){
				sql += " where " + where;
			}
			db.Parameters.Clear();
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [tblHistory]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "tblHistory");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "tblHistory");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "tblHistory",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.tblHistory GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [tblHistory] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.tblHistory Entity = new ADeeWu.HuoBi3J.Model.tblHistory();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.FromUIN = (string)(dr["FromUIN"]);
			Entity.ToUIN = (string)(dr["ToUIN"]);
			Entity.nTime = int.Parse(dr["nTime"].ToString());
			Entity.sMsg = (string)(dr["sMsg"]);
			Entity.nType = int.Parse(dr["nType"].ToString());
			Entity.GroupChatID = long.Parse(dr["GroupChatID"].ToString());
			Entity.sIP = (string)(dr["sIP"]);
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.tblHistory GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.tblHistory[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.tblHistory[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [tblHistory] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.tblHistory[] EntityList = new ADeeWu.HuoBi3J.Model.tblHistory[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"tblHistory","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"tblHistory","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"tblHistory","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"tblHistory","ID",where,orderBy);
		}
