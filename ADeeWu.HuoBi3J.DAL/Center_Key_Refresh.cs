...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_Key_Refresh{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_Key_Refresh] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_Key_Refresh model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@LastTime",model.LastTime.HasValue ? (object)model.LastTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Center_Key_Refresh]([KID],[UID],[LastTime]) values (@KID,@UID,@LastTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_Key_Refresh model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@UID",model.UID.HasValue ? (object)model.UID.Value : (object)DBNull.Value );
		db.Parameters.Append("@LastTime",model.LastTime.HasValue ? (object)model.LastTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Center_Key_Refresh] set [KID]=@KID,[UID]=@UID,[LastTime]=@LastTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_Key_Refresh",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_Key_Refresh");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_Key_Refresh]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_Key_Refresh");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_Key_Refresh");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_Key_Refresh",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Refresh GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_Key_Refresh] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_Key_Refresh Entity = new ADeeWu.HuoBi3J.Model.Center_Key_Refresh();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.KID = dr["KID"] as long?;
			Entity.UID = dr["UID"] as long?;
			Entity.LastTime = dr["LastTime"] as DateTime?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Refresh GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_Key_Refresh[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Refresh GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_Key_Refresh[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_Key_Refresh] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_Key_Refresh[] EntityList = new ADeeWu.HuoBi3J.Model.Center_Key_Refresh[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_Key_Refresh","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Refresh","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Refresh","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Refresh","ID",where,orderBy);
		}
