...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_AuctionLog{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_AuctionLog] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_AuctionLog model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@AuctionID",model.AuctionID.HasValue ? (object)model.AuctionID.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@IsWin",model.IsWin.HasValue ? (object)model.IsWin.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Forum_AuctionLog]([AuctionID],[UserID],[DateTime],[Score],[IsWin]) values (@AuctionID,@UserID,@DateTime,@Score,@IsWin);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_AuctionLog model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Score",model.Score.HasValue ? (object)model.Score.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsWin",model.IsWin.HasValue ? (object)model.IsWin.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Forum_AuctionLog] set [AuctionID]=@AuctionID,[UserID]=@UserID,[DateTime]=@DateTime,[Score]=@Score,[IsWin]=@IsWin where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_AuctionLog",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_AuctionLog");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_AuctionLog]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_AuctionLog");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_AuctionLog");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_AuctionLog",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_AuctionLog GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_AuctionLog] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_AuctionLog Entity = new ADeeWu.HuoBi3J.Model.Forum_AuctionLog();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.AuctionID = dr["AuctionID"] as long?;
			Entity.UserID = dr["UserID"] as long?;
			Entity.DateTime = dr["DateTime"] as DateTime?;
			Entity.Score = dr["Score"] as int?;
			Entity.IsWin = dr["IsWin"] as bool?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Forum_AuctionLog GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_AuctionLog[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_AuctionLog GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_AuctionLog[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_AuctionLog] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_AuctionLog[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_AuctionLog[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_AuctionLog","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AuctionLog","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AuctionLog","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_AuctionLog","ID",where,orderBy);
		}
