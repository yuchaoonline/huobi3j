...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_MyFans{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_MyFans] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_MyFans model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@FansID",model.FansID.HasValue ? (object)model.FansID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateDate",model.CreateDate.HasValue ? (object)model.CreateDate.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Forum_MyFans]([UserID],[FansID],[CreateDate]) values (@UserID,@FansID,@CreateDate);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_MyFans model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@FansID",model.FansID.HasValue ? (object)model.FansID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateDate",model.CreateDate.HasValue ? (object)model.CreateDate.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Forum_MyFans] set [UserID]=@UserID,[FansID]=@FansID,[CreateDate]=@CreateDate where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_MyFans",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_MyFans");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_MyFans]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_MyFans");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_MyFans");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_MyFans",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_MyFans GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_MyFans] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_MyFans Entity = new ADeeWu.HuoBi3J.Model.Forum_MyFans();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = dr["UserID"] as long?;
			Entity.FansID = dr["FansID"] as long?;
			Entity.CreateDate = dr["CreateDate"] as DateTime?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Forum_MyFans GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_MyFans[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_MyFans GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_MyFans[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_MyFans] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_MyFans[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_MyFans[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_MyFans","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_MyFans","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_MyFans","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_MyFans","ID",where,orderBy);
		}
