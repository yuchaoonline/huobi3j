...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Key_ViewPrice_Log{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Key_ViewPrice_Log] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CountClickID",model.CountClickID.HasValue ? (object)model.CountClickID.Value : (object)DBNull.Value );
		db.Parameters.Append("@Price",model.Price.HasValue ? (object)model.Price.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Key_ViewPrice_Log]([CountClickID],[Price]) values (@CountClickID,@Price);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@CountClickID",model.CountClickID.HasValue ? (object)model.CountClickID.Value : (object)DBNull.Value );
		db.Parameters.Append("@Price",model.Price.HasValue ? (object)model.Price.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Key_ViewPrice_Log] set [CountClickID]=@CountClickID,[Price]=@Price where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Key_ViewPrice_Log",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Key_ViewPrice_Log");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Key_ViewPrice_Log]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Key_ViewPrice_Log");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Key_ViewPrice_Log");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Key_ViewPrice_Log",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Key_ViewPrice_Log] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log Entity = new ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.CountClickID = dr["CountClickID"] as int?;
			Entity.Price = dr["Price"] as decimal?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Key_ViewPrice_Log] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log[] EntityList = new ADeeWu.HuoBi3J.Model.Key_ViewPrice_Log[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Key_ViewPrice_Log","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice_Log","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice_Log","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice_Log","ID",where,orderBy);
		}
