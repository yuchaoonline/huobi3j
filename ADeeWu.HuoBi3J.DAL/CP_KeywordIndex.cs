...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_KeywordIndex{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_KeywordIndex] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_KeywordIndex model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Keyword",model.Keyword);
		db.Parameters.Append("@PromotionID",model.PromotionID);
		db.Parameters.Append("@IsHidden",model.IsHidden);
		db.Parameters.Append("@IsRankIndex",model.IsRankIndex);
		db.Parameters.Append("@Sequence",model.Sequence);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [CP_KeywordIndex]([Keyword],[PromotionID],[IsHidden],[IsRankIndex],[Sequence],[CreateTime]) values (@Keyword,@PromotionID,@IsHidden,@IsRankIndex,@Sequence,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_KeywordIndex model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Sequence",model.Sequence);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [CP_KeywordIndex] set [Keyword]=@Keyword,[PromotionID]=@PromotionID,[IsHidden]=@IsHidden,[IsRankIndex]=@IsRankIndex,[Sequence]=@Sequence,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_KeywordIndex",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_KeywordIndex");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_KeywordIndex]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_KeywordIndex");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_KeywordIndex");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_KeywordIndex",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_KeywordIndex GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_KeywordIndex] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_KeywordIndex Entity = new ADeeWu.HuoBi3J.Model.CP_KeywordIndex();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.Keyword = (string)(dr["Keyword"]);
			Entity.PromotionID = long.Parse(dr["PromotionID"].ToString());
			Entity.IsHidden = bool.Parse(dr["IsHidden"].ToString());
			Entity.IsRankIndex = bool.Parse(dr["IsRankIndex"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CP_KeywordIndex GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_KeywordIndex[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_KeywordIndex GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_KeywordIndex[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_KeywordIndex] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_KeywordIndex[] EntityList = new ADeeWu.HuoBi3J.Model.CP_KeywordIndex[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_KeywordIndex","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_KeywordIndex","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_KeywordIndex","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_KeywordIndex","ID",where,orderBy);
		}
