...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Job_Categories{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Job_Categories] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Job_Categories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Job_Categories]([Name],[ItemCount],[ParentID],[ParentPath],[Depth],[IsRecommend],[IsHidden],[Sequence],[CreateTime],[ModifyTime]) values (@Name,@ItemCount,@ParentID,@ParentPath,@Depth,@IsRecommend,@IsHidden,@Sequence,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Job_Categories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Job_Categories] set [Name]=@Name,[ItemCount]=@ItemCount,[ParentID]=@ParentID,[ParentPath]=@ParentPath,[Depth]=@Depth,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[Sequence]=@Sequence,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Job_Categories",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Job_Categories");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Job_Categories]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Job_Categories");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Job_Categories");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Job_Categories",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Job_Categories GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Job_Categories] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Job_Categories Entity = new ADeeWu.HuoBi3J.Model.Job_Categories();
		...
		public ADeeWu.HuoBi3J.Model.Job_Categories GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Job_Categories[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Job_Categories GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Job_Categories[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Job_Categories] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Job_Categories[] EntityList = new ADeeWu.HuoBi3J.Model.Job_Categories[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Job_Categories","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_Categories","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_Categories","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_Categories","ID",where,orderBy);
		}
