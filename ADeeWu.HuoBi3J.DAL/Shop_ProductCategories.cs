...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Shop_ProductCategories{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Shop_ProductCategories model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@AttrTplCateID",model.AttrTplCateID.HasValue ? (object)model.AttrTplCateID.Value : (object)DBNull.Value );
		db.Parameters.Append("@Name",model.Name );
		db.Parameters.Append("@ItemCount",model.ItemCount.HasValue ? (object)model.ItemCount.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@Depth",model.Depth.HasValue ? (object)model.Depth.Value : (object)DBNull.Value );
		db.Parameters.Append("@Sequence",model.Sequence.HasValue ? (object)model.Sequence.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsRecommend",model.IsRecommend.HasValue ? (object)model.IsRecommend.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@ID",ID);
			...
			Entity.AttrTplCateID = dr["AttrTplCateID"] as long?;
			Entity.Name = dr["Name"] as string;
			Entity.ItemCount = dr["ItemCount"] as int?;
			Entity.ParentID = dr["ParentID"] as long?;
			Entity.ParentPath = dr["ParentPath"] as string;
			Entity.Depth = dr["Depth"] as int?;
			Entity.Sequence = dr["Sequence"] as int?;
			Entity.IsRecommend = dr["IsRecommend"] as bool?;
