...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class BaseData{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.BaseData model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Name",model.Name );
		db.Parameters.Append("@Value",model.Value );
		db.Parameters.Append("@IDentity",model.IDentity.HasValue ? (object)model.IDentity.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [BaseData]([Name],[Value],[IDentity]) values (@Name,@Value,@IDentity);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.BaseData model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Value",model.Value );
		db.Parameters.Append("@IDentity",model.IDentity.HasValue ? (object)model.IDentity.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [BaseData] set [Name]=@Name,[Value]=@Value,[IDentity]=@IDentity where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","BaseData",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","BaseData");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [BaseData]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "BaseData");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "BaseData");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "BaseData",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.BaseData GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.BaseData[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"BaseData","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"BaseData","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"BaseData","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"BaseData","ID",where,orderBy);
		}
