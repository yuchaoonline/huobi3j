...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CT_CashTickets{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CT_CashTickets] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CT_CashTickets model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Money",model.Money);
		    DataTable dt = db.Select("insert into [CT_CashTickets]([SerialNum],[ValidCode],[CorpID],[Summary],[CreateTime],[CheckState],[ModifyTime],[Money]) values (@SerialNum,@ValidCode,@CorpID,@Summary,@CreateTime,@CheckState,@ModifyTime,@Money);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CT_CashTickets model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		db.Parameters.Append("@Money",model.Money);
			return db.ExecuteSql("update [CT_CashTickets] set [SerialNum]=@SerialNum,[ValidCode]=@ValidCode,[CorpID]=@CorpID,[Summary]=@Summary,[CreateTime]=@CreateTime,[CheckState]=@CheckState,[ModifyTime]=@ModifyTime,[Money]=@Money where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CT_CashTickets",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CT_CashTickets");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CT_CashTickets]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CT_CashTickets");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CT_CashTickets");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CT_CashTickets",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CT_CashTickets GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CT_CashTickets] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CT_CashTickets Entity = new ADeeWu.HuoBi3J.Model.CT_CashTickets();
			...
			Entity.Money = double.Parse(dr["Money"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.CT_CashTickets GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CT_CashTickets[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CT_CashTickets GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CT_CashTickets[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CT_CashTickets] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CT_CashTickets[] EntityList = new ADeeWu.HuoBi3J.Model.CT_CashTickets[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CT_CashTickets","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTickets","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTickets","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTickets","ID",where,orderBy);
		}
