...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Key_ViewPrice{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Key_ViewPrice] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Key_ViewPrice model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Count",model.Count.HasValue ? (object)model.Count.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Key_ViewPrice]([KID],[Price],[Count]) values (@KID,@Price,@Count);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Key_ViewPrice model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Price",model.Price.HasValue ? (object)model.Price.Value : (object)DBNull.Value );
		db.Parameters.Append("@Count",model.Count.HasValue ? (object)model.Count.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Key_ViewPrice] set [KID]=@KID,[Price]=@Price,[Count]=@Count where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Key_ViewPrice",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Key_ViewPrice");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Key_ViewPrice]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Key_ViewPrice");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Key_ViewPrice");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Key_ViewPrice",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Key_ViewPrice] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Key_ViewPrice Entity = new ADeeWu.HuoBi3J.Model.Key_ViewPrice();
		...
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Key_ViewPrice[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Key_ViewPrice[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Key_ViewPrice] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Key_ViewPrice[] EntityList = new ADeeWu.HuoBi3J.Model.Key_ViewPrice[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Key_ViewPrice","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_ViewPrice","ID",where,orderBy);
		}
