...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_Users{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Admin_Users] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_Users model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Notes",model.Notes);
		    DataTable dt = db.Select("insert into [Admin_Users]([LoginName],[LoginPwd],[RegTime],[LastLogin],[LoginTimes],[Name],[Notes]) values (@LoginName,@LoginPwd,@RegTime,@LastLogin,@LoginTimes,@Name,@Notes);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_Users model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@Notes",model.Notes);
			return db.ExecuteSql("update [Admin_Users] set [LoginName]=@LoginName,[LoginPwd]=@LoginPwd,[RegTime]=@RegTime,[LastLogin]=@LastLogin,[LoginTimes]=@LoginTimes,[Name]=@Name,[Notes]=@Notes where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_Users",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_Users");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_Users]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_Users");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_Users");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_Users",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Admin_Users GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Admin_Users] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Admin_Users Entity = new ADeeWu.HuoBi3J.Model.Admin_Users();
		...
		public ADeeWu.HuoBi3J.Model.Admin_Users GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_Users[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Admin_Users GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Admin_Users[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Admin_Users] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Admin_Users[] EntityList = new ADeeWu.HuoBi3J.Model.Admin_Users[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Admin_Users","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Users","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Users","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Users","ID",where,orderBy);
		}
