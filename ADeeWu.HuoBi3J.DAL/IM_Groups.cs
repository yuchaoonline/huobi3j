...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_Groups{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_Groups] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_Groups model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UIN",model.UIN);
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@AdminUIN",model.AdminUIN);
		db.Parameters.Append("@AdminUserID",model.AdminUserID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_Groups]([UIN],[Name],[AdminUIN],[AdminUserID],[CreateTime]) values (@UIN,@Name,@AdminUIN,@AdminUserID,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_Groups model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@AdminUserID",model.AdminUserID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_Groups] set [UIN]=@UIN,[Name]=@Name,[AdminUIN]=@AdminUIN,[AdminUserID]=@AdminUserID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_Groups",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_Groups");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_Groups]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_Groups");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_Groups");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_Groups",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_Groups GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_Groups] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_Groups Entity = new ADeeWu.HuoBi3J.Model.IM_Groups();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UIN = (string)(dr["UIN"]);
			Entity.Name = (string)(dr["Name"]);
			Entity.AdminUIN = (string)(dr["AdminUIN"]);
		...
		public ADeeWu.HuoBi3J.Model.IM_Groups GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_Groups[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_Groups GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_Groups[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_Groups] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_Groups[] EntityList = new ADeeWu.HuoBi3J.Model.IM_Groups[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_Groups","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Groups","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Groups","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Groups","ID",where,orderBy);
		}
