...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Job_SeekerFavs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Job_SeekerFavs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Job_SeekerFavs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@JobID",model.JobID);
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [Job_SeekerFavs]([UserID],[JobID],[Notes],[CreateTime]) values (@UserID,@JobID,@Notes,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Job_SeekerFavs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [Job_SeekerFavs] set [UserID]=@UserID,[JobID]=@JobID,[Notes]=@Notes,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Job_SeekerFavs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Job_SeekerFavs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Job_SeekerFavs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Job_SeekerFavs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Job_SeekerFavs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Job_SeekerFavs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Job_SeekerFavs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Job_SeekerFavs] where 1=1  and [ID]=@ID");
			...
			Entity.JobID = long.Parse(dr["JobID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Job_SeekerFavs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Job_SeekerFavs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Job_SeekerFavs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Job_SeekerFavs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Job_SeekerFavs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Job_SeekerFavs[] EntityList = new ADeeWu.HuoBi3J.Model.Job_SeekerFavs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Job_SeekerFavs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_SeekerFavs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_SeekerFavs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Job_SeekerFavs","ID",where,orderBy);
		}
