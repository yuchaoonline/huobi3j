...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Common_Count_Click{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Common_Count_Click] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Common_Count_Click model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IP",model.IP );
		    DataTable dt = db.Select("insert into [Common_Count_Click]([CreateDate],[DataType],[DataID],[IP]) values (@CreateDate,@DataType,@DataID,@IP);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Common_Count_Click model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@DataID",model.DataID.HasValue ? (object)model.DataID.Value : (object)DBNull.Value );
		db.Parameters.Append("@IP",model.IP );
			return db.ExecuteSql("update [Common_Count_Click] set [CreateDate]=@CreateDate,[DataType]=@DataType,[DataID]=@DataID,[IP]=@IP where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Common_Count_Click",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Common_Count_Click");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Common_Count_Click]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Common_Count_Click");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Common_Count_Click");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Common_Count_Click",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Common_Count_Click GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Common_Count_Click] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Common_Count_Click Entity = new ADeeWu.HuoBi3J.Model.Common_Count_Click();
		...
		public ADeeWu.HuoBi3J.Model.Common_Count_Click GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Common_Count_Click[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Common_Count_Click GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Common_Count_Click[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Common_Count_Click] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Common_Count_Click[] EntityList = new ADeeWu.HuoBi3J.Model.Common_Count_Click[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Common_Count_Click","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Common_Count_Click","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Common_Count_Click","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Common_Count_Click","ID",where,orderBy);
		}
