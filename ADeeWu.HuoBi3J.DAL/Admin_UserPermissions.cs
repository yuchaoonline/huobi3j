...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_UserPermissions{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_UserPermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CheckState",model.CheckState);
		    DataTable dt = db.Select("insert into [Admin_UserPermissions]([PageID],[AdminID],[CheckState]) values (@PageID,@AdminID,@CheckState);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_UserPermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@AdminID",model.AdminID.HasValue ? (object)model.AdminID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CheckState",model.CheckState);
			return db.ExecuteSql("update [Admin_UserPermissions] set [PageID]=@PageID,[AdminID]=@AdminID,[CheckState]=@CheckState where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_UserPermissions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_UserPermissions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_UserPermissions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_UserPermissions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_UserPermissions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_UserPermissions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.Admin_UserPermissions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_UserPermissions[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"Admin_UserPermissions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserPermissions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserPermissions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserPermissions","ID",where,orderBy);
		}
