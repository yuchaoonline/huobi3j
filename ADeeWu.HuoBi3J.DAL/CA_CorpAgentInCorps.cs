...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_CorpAgentInCorps{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_CorpAgentInCorps] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [CA_CorpAgentInCorps]([CorpAgentID],[CorpID],[CreateTime]) values (@CorpAgentID,@CorpID,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CorpID",model.CorpID);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [CA_CorpAgentInCorps] set [CorpAgentID]=@CorpAgentID,[CorpID]=@CorpID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_CorpAgentInCorps",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_CorpAgentInCorps");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_CorpAgentInCorps]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_CorpAgentInCorps");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_CorpAgentInCorps");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_CorpAgentInCorps",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_CorpAgentInCorps] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps Entity = new ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps();
		...
		public ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_CorpAgentInCorps] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps[] EntityList = new ADeeWu.HuoBi3J.Model.CA_CorpAgentInCorps[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_CorpAgentInCorps","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_CorpAgentInCorps","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_CorpAgentInCorps","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_CorpAgentInCorps","ID",where,orderBy);
		}
