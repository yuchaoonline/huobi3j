...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CT_CashTicketApplications{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CT_CashTicketApplications] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CT_CashTicketApplications model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@CorpID",model.CorpID);
		db.Parameters.Append("@CashTicketID",model.CashTicketID);
		db.Parameters.Append("@SerialNum",model.SerialNum);
		db.Parameters.Append("@ValidCode",model.ValidCode);
		db.Parameters.Append("@CorpName",model.CorpName);
		db.Parameters.Append("@CostDate",model.CostDate);
		db.Parameters.Append("@ReturnMoney",model.ReturnMoney);
		...
		db.Parameters.Append("@Notes",model.Notes);
		    DataTable dt = db.Select("insert into [CT_CashTicketApplications]([UserID],[CorpID],[CashTicketID],[SerialNum],[ValidCode],[CorpName],[CostDate],[ReturnMoney],[Summary],[CheckState],[CreateTime],[ModifyTime],[Notes]) values (@UserID,@CorpID,@CashTicketID,@SerialNum,@ValidCode,@CorpName,@CostDate,@ReturnMoney,@Summary,@CheckState,@CreateTime,@ModifyTime,@Notes);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CT_CashTicketApplications model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		db.Parameters.Append("@Notes",model.Notes);
			return db.ExecuteSql("update [CT_CashTicketApplications] set [UserID]=@UserID,[CorpID]=@CorpID,[CashTicketID]=@CashTicketID,[SerialNum]=@SerialNum,[ValidCode]=@ValidCode,[CorpName]=@CorpName,[CostDate]=@CostDate,[ReturnMoney]=@ReturnMoney,[Summary]=@Summary,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime,[Notes]=@Notes where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CT_CashTicketApplications",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CT_CashTicketApplications");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CT_CashTicketApplications]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CT_CashTicketApplications");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CT_CashTicketApplications");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CT_CashTicketApplications",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CT_CashTicketApplications GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CT_CashTicketApplications] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CT_CashTicketApplications Entity = new ADeeWu.HuoBi3J.Model.CT_CashTicketApplications();
			...
			Entity.CashTicketID = long.Parse(dr["CashTicketID"].ToString());
			Entity.SerialNum = (string)(dr["SerialNum"]);
			Entity.ValidCode = (string)(dr["ValidCode"]);
			Entity.CorpName = (string)(dr["CorpName"]);
			Entity.CostDate = DateTime.Parse(dr["CostDate"].ToString());
			Entity.ReturnMoney = decimal.Parse(dr["ReturnMoney"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CT_CashTicketApplications GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CT_CashTicketApplications[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CT_CashTicketApplications GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CT_CashTicketApplications[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CT_CashTicketApplications] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CT_CashTicketApplications[] EntityList = new ADeeWu.HuoBi3J.Model.CT_CashTicketApplications[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CT_CashTicketApplications","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTicketApplications","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTicketApplications","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CT_CashTicketApplications","ID",where,orderBy);
		}
