...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Market_Infos{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Market_Infos] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Market_Infos model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@MarketCategoryID",model.MarketCategoryID);
		...
		db.Parameters.Append("@BusinessType",model.BusinessType);
		    DataTable dt = db.Select("insert into [Market_Infos]([UserID],[MarketCategoryID],[Title],[Summary],[Content],[IsRecommend],[IsHidden],[VisitCount],[CheckState],[CreateTime],[ModifyTime],[RefreshTime],[BusinessType]) values (@UserID,@MarketCategoryID,@Title,@Summary,@Content,@IsRecommend,@IsHidden,@VisitCount,@CheckState,@CreateTime,@ModifyTime,@RefreshTime,@BusinessType);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Market_Infos model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@UserID",model.UserID);
		...
		db.Parameters.Append("@RefreshTime",model.RefreshTime);
		db.Parameters.Append("@BusinessType",model.BusinessType);
			return db.ExecuteSql("update [Market_Infos] set [UserID]=@UserID,[MarketCategoryID]=@MarketCategoryID,[Title]=@Title,[Summary]=@Summary,[Content]=@Content,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[VisitCount]=@VisitCount,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime,[RefreshTime]=@RefreshTime,[BusinessType]=@BusinessType where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Market_Infos",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] set 1=1","Market_Infos");
			if(!string.IsNullOrEmpty(where)){
				sql += " where " + where;
			}
			db.Parameters.Clear();
			...
			if(builder.Length>0)
			{
				sql += builder.ToString().Substring(1);
			}
			return db.ExecuteSql(sql);
		}
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Market_Infos]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Market_Infos");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Market_Infos");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Market_Infos",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Market_Infos GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Market_Infos] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Market_Infos Entity = new ADeeWu.HuoBi3J.Model.Market_Infos();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.MarketCategoryID = long.Parse(dr["MarketCategoryID"].ToString());
			Entity.Title = (string)(dr["Title"].ToString());
			Entity.Summary = (string)(dr["Summary"].ToString());
			Entity.Content = (string)(dr["Content"].ToString());
			...
			Entity.BusinessType = int.Parse(dr["BusinessType"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Market_Infos GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Market_Infos[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Market_Infos[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Market_Infos] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Market_Infos[] EntityList = new ADeeWu.HuoBi3J.Model.Market_Infos[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Market_Infos","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_Infos","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_Infos","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Market_Infos","ID",where,orderBy);
		}
