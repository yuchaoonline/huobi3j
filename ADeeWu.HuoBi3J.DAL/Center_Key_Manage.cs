...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_Key_Manage{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_Key_Manage] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_Key_Manage model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreatTime",model.CreatTime.HasValue ? (object)model.CreatTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@Price",model.Price.HasValue ? (object)model.Price.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsGoOn",model.IsGoOn.HasValue ? (object)model.IsGoOn.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Center_Key_Manage]([UID],[KID],[CreatTime],[Price],[IsGoOn]) values (@UID,@KID,@CreatTime,@Price,@IsGoOn);select @@Identity;");
            int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_Key_Manage model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Price",model.Price.HasValue ? (object)model.Price.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsGoOn",model.IsGoOn.HasValue ? (object)model.IsGoOn.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Center_Key_Manage] set [UID]=@UID,[KID]=@KID,[CreatTime]=@CreatTime,[Price]=@Price,[IsGoOn]=@IsGoOn where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_Key_Manage",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_Key_Manage");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_Key_Manage]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_Key_Manage");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_Key_Manage");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_Key_Manage",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Manage GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_Key_Manage] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_Key_Manage Entity = new ADeeWu.HuoBi3J.Model.Center_Key_Manage();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.UID = dr["UID"] as int?;
			Entity.KID = dr["KID"] as int?;
			Entity.CreatTime = dr["CreatTime"] as DateTime?;
			Entity.Price = dr["Price"] as double?;
			Entity.IsGoOn = dr["IsGoOn"] as bool?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Manage GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_Key_Manage[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Manage GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_Key_Manage[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_Key_Manage] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_Key_Manage[] EntityList = new ADeeWu.HuoBi3J.Model.Center_Key_Manage[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_Key_Manage","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Manage","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Manage","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Manage","ID",where,orderBy);
		}
