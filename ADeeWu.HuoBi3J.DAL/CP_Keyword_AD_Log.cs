...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_Keyword_AD_Log{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_Keyword_AD_Log] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ClickIP",model.ClickIP );
		db.Parameters.Append("@ClickPrice",model.ClickPrice );
		db.Parameters.Append("@ADID",model.ADID.HasValue ? (object)model.ADID.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [CP_Keyword_AD_Log]([ClickIP],[ClickPrice],[ADID],[KeywordID],[CreateTime]) values (@ClickIP,@ClickPrice,@ADID,@KeywordID,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@KeywordID",model.KeywordID.HasValue ? (object)model.KeywordID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [CP_Keyword_AD_Log] set [ClickIP]=@ClickIP,[ClickPrice]=@ClickPrice,[ADID]=@ADID,[KeywordID]=@KeywordID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_Keyword_AD_Log",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_Keyword_AD_Log");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_Keyword_AD_Log]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_Keyword_AD_Log");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_Keyword_AD_Log");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_Keyword_AD_Log",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_Keyword_AD_Log] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log Entity = new ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.ClickIP = dr["ClickIP"] as string;
			Entity.ClickPrice = dr["ClickPrice"] as decimal?;
			Entity.ADID = dr["ADID"] as long?;
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_Keyword_AD_Log] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log[] EntityList = new ADeeWu.HuoBi3J.Model.CP_Keyword_AD_Log[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_Keyword_AD_Log","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD_Log","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD_Log","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Keyword_AD_Log","ID",where,orderBy);
		}
