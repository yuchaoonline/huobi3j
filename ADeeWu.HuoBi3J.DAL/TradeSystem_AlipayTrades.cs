...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class TradeSystem_AlipayTrades{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [TradeSystem_AlipayTrades] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@OrderCode",model.OrderCode);
		db.Parameters.Append("@AlipayOrder",model.AlipayOrder);
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@TotalFee",model.TotalFee);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [TradeSystem_AlipayTrades]([OrderCode],[AlipayOrder],[UserID],[TotalFee],[Notes],[OrderState],[CreateTime],[ModifyTime]) values (@OrderCode,@AlipayOrder,@UserID,@TotalFee,@Notes,@OrderState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [TradeSystem_AlipayTrades] set [OrderCode]=@OrderCode,[AlipayOrder]=@AlipayOrder,[UserID]=@UserID,[TotalFee]=@TotalFee,[Notes]=@Notes,[OrderState]=@OrderState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","TradeSystem_AlipayTrades",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","TradeSystem_AlipayTrades");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [TradeSystem_AlipayTrades]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "TradeSystem_AlipayTrades");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "TradeSystem_AlipayTrades");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "TradeSystem_AlipayTrades",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [TradeSystem_AlipayTrades] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades Entity = new ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.OrderCode = (string)(dr["OrderCode"]);
			Entity.AlipayOrder = (string)(dr["AlipayOrder"]);
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.TotalFee = decimal.Parse(dr["TotalFee"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [TradeSystem_AlipayTrades] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades[] EntityList = new ADeeWu.HuoBi3J.Model.TradeSystem_AlipayTrades[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"TradeSystem_AlipayTrades","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_AlipayTrades","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_AlipayTrades","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_AlipayTrades","ID",where,orderBy);
		}
