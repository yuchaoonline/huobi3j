...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_Products{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_Products] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_Products model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CategoryID",model.CategoryID.HasValue ? (object)model.CategoryID.Value : (object)DBNull.Value );
		db.Parameters.Append("@Name",model.Name );
		db.Parameters.Append("@Content",model.Content );
		db.Parameters.Append("@Picture",model.Picture );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Center_Products]([CategoryID],[Name],[Content],[Picture],[CreateTime]) values (@CategoryID,@Name,@Content,@Picture,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_Products model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Picture",model.Picture );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Center_Products] set [CategoryID]=@CategoryID,[Name]=@Name,[Content]=@Content,[Picture]=@Picture,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_Products",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_Products");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_Products]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_Products");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_Products");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_Products",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_Products GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_Products] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_Products Entity = new ADeeWu.HuoBi3J.Model.Center_Products();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CategoryID = dr["CategoryID"] as long?;
			Entity.Name = dr["Name"] as string;
			Entity.Content = dr["Content"] as string;
			Entity.Picture = dr["Picture"] as string;
			Entity.CreateTime = dr["CreateTime"] as DateTime?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Center_Products GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_Products[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_Products GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_Products[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_Products] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_Products[] EntityList = new ADeeWu.HuoBi3J.Model.Center_Products[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_Products","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Products","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Products","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Products","ID",where,orderBy);
		}
