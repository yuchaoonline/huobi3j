...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_AfterSaleRecords{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_AfterSaleRecords] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@OrderDetailID",model.OrderDetailID);
		db.Parameters.Append("@ServerCorpID",model.ServerCorpID);
		db.Parameters.Append("@ServerCorpAgentID",model.ServerCorpAgentID);
		db.Parameters.Append("@Result",model.Result);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_AfterSaleRecords]([OrderDetailID],[ServerCorpID],[ServerCorpAgentID],[Result],[Note],[CreateTime],[ModifyTime]) values (@OrderDetailID,@ServerCorpID,@ServerCorpAgentID,@Result,@Note,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_AfterSaleRecords] set [OrderDetailID]=@OrderDetailID,[ServerCorpID]=@ServerCorpID,[ServerCorpAgentID]=@ServerCorpAgentID,[Result]=@Result,[Note]=@Note,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_AfterSaleRecords",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_AfterSaleRecords");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_AfterSaleRecords]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_AfterSaleRecords");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_AfterSaleRecords");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_AfterSaleRecords",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_AfterSaleRecords] where 1=1  and [ID]=@ID");
			...
			Entity.OrderDetailID = long.Parse(dr["OrderDetailID"].ToString());
			Entity.ServerCorpID = long.Parse(dr["ServerCorpID"].ToString());
			Entity.ServerCorpAgentID = long.Parse(dr["ServerCorpAgentID"].ToString());
			Entity.Result = int.Parse(dr["Result"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_AfterSaleRecords] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_AfterSaleRecords[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_AfterSaleRecords","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_AfterSaleRecords","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_AfterSaleRecords","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_AfterSaleRecords","ID",where,orderBy);
		}
