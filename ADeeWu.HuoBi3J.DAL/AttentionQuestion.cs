...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class AttentionQuestion{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [AttentionQuestion] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.AttentionQuestion model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IsRead",model.IsRead.HasValue ? (object)model.IsRead.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsReply",model.IsReply.HasValue ? (object)model.IsReply.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [AttentionQuestion]([UID],[QID],[IsRead],[CreateTime],[IsReply]) values (@UID,@QID,@IsRead,@CreateTime,@IsReply);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.AttentionQuestion model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@IsReply",model.IsReply.HasValue ? (object)model.IsReply.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [AttentionQuestion] set [UID]=@UID,[QID]=@QID,[IsRead]=@IsRead,[CreateTime]=@CreateTime,[IsReply]=@IsReply where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","AttentionQuestion",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","AttentionQuestion");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [AttentionQuestion]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "AttentionQuestion");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "AttentionQuestion");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "AttentionQuestion",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.AttentionQuestion GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [AttentionQuestion] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.AttentionQuestion Entity = new ADeeWu.HuoBi3J.Model.AttentionQuestion();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UID = dr["UID"] as long?;
			Entity.QID = dr["QID"] as long?;
			Entity.IsRead = dr["IsRead"] as bool?;
			Entity.CreateTime = dr["CreateTime"] as DateTime?;
			Entity.IsReply = dr["IsReply"] as bool?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.AttentionQuestion GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.AttentionQuestion[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.AttentionQuestion GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.AttentionQuestion[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [AttentionQuestion] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.AttentionQuestion[] EntityList = new ADeeWu.HuoBi3J.Model.AttentionQuestion[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"AttentionQuestion","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"AttentionQuestion","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"AttentionQuestion","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"AttentionQuestion","ID",where,orderBy);
		}
