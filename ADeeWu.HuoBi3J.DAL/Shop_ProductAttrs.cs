...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_ProductAttrs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_ProductAttrs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_ProductAttrs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ProductID",model.ProductID);
		db.Parameters.Append("@AttrID",model.AttrID);
		db.Parameters.Append("@AttrOptionIDList",model.AttrOptionIDList);
		db.Parameters.Append("@CustomValue",model.CustomValue);
		    DataTable dt = db.Select("insert into [Shop_ProductAttrs]([ProductID],[AttrID],[AttrOptionIDList],[CustomValue]) values (@ProductID,@AttrID,@AttrOptionIDList,@CustomValue);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_ProductAttrs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@AttrOptionIDList",model.AttrOptionIDList);
		db.Parameters.Append("@CustomValue",model.CustomValue);
			return db.ExecuteSql("update [Shop_ProductAttrs] set [ProductID]=@ProductID,[AttrID]=@AttrID,[AttrOptionIDList]=@AttrOptionIDList,[CustomValue]=@CustomValue where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_ProductAttrs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_ProductAttrs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_ProductAttrs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_ProductAttrs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_ProductAttrs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_ProductAttrs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_ProductAttrs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_ProductAttrs] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_ProductAttrs Entity = new ADeeWu.HuoBi3J.Model.Shop_ProductAttrs();
			...
			Entity.AttrOptionIDList = (string)(dr["AttrOptionIDList"]);
			Entity.CustomValue = (string)(dr["CustomValue"]);
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Shop_ProductAttrs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_ProductAttrs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_ProductAttrs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_ProductAttrs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_ProductAttrs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_ProductAttrs[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_ProductAttrs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_ProductAttrs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_ProductAttrs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_ProductAttrs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_ProductAttrs","ID",where,orderBy);
		}
