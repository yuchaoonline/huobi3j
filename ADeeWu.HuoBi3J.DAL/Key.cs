...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Key{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Key] where 1=1");
		...
		public int Update(ADeeWu.HuoBi3J.Model.Key model)
		{
			db.Parameters.Clear();
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Key",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Key");
		...
		public int Delete(long  KID)
		{
			...
			return db.ExecuteSql("delete from [Key]  where  @KID=KID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Key");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Key");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Key",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Key GetEntity(long  KID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@KID",KID);
			DataTable dt = db.Select("select * from [Key] where 1=1  and [KID]=@KID");
		...
		public ADeeWu.HuoBi3J.Model.Key GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Key[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Key GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Key[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Key] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Key[] EntityList = new ADeeWu.HuoBi3J.Model.Key[dt.Rows.Count];
