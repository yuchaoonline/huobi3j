...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_Products{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_Products] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_Products model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ShopID",model.ShopID);
		...
		db.Parameters.Append("@Stock",model.Stock);
		db.Parameters.Append("@SaleOut",model.SaleOut);
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@Summary",model.Summary);
		db.Parameters.Append("@Keywords",model.Keywords);
		db.Parameters.Append("@Content",model.Content);
		db.Parameters.Append("@Price",model.Price);
		db.Parameters.Append("@ReturnPrecent",model.ReturnPrecent);
		db.Parameters.Append("@UsePriceForReturn",model.UsePriceForReturn);
		db.Parameters.Append("@FloatPrice",model.FloatPrice);
		db.Parameters.Append("@Picture0",model.Picture0);
		db.Parameters.Append("@Picture1",model.Picture1);
		db.Parameters.Append("@PictureList",model.PictureList);
		db.Parameters.Append("@SaleTime",model.SaleTime);
		db.Parameters.Append("@SaleAddress",model.SaleAddress);
		db.Parameters.Append("@HasContract",model.HasContract);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_Products]([ShopID],[CorpID],[CorpUserID],[CategoryID],[Stock],[SaleOut],[Name],[Summary],[Keywords],[Content],[Price],[ReturnPrecent],[UsePriceForReturn],[FloatPrice],[Picture0],[Picture1],[PictureList],[SaleTime],[SaleAddress],[HasContract],[ProvinceID],[CityID],[AreaID],[Province],[City],[Area],[IsHidden],[IsRecommend],[Sequence],[VisitCount],[CheckState],[CreateTime],[ModifyTime]) values (@ShopID,@CorpID,@CorpUserID,@CategoryID,@Stock,@SaleOut,@Name,@Summary,@Keywords,@Content,@Price,@ReturnPrecent,@UsePriceForReturn,@FloatPrice,@Picture0,@Picture1,@PictureList,@SaleTime,@SaleAddress,@HasContract,@ProvinceID,@CityID,@AreaID,@Province,@City,@Area,@IsHidden,@IsRecommend,@Sequence,@VisitCount,@CheckState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_Products model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_Products] set [ShopID]=@ShopID,[CorpID]=@CorpID,[CorpUserID]=@CorpUserID,[CategoryID]=@CategoryID,[Stock]=@Stock,[SaleOut]=@SaleOut,[Name]=@Name,[Summary]=@Summary,[Keywords]=@Keywords,[Content]=@Content,[Price]=@Price,[ReturnPrecent]=@ReturnPrecent,[UsePriceForReturn]=@UsePriceForReturn,[FloatPrice]=@FloatPrice,[Picture0]=@Picture0,[Picture1]=@Picture1,[PictureList]=@PictureList,[SaleTime]=@SaleTime,[SaleAddress]=@SaleAddress,[HasContract]=@HasContract,[ProvinceID]=@ProvinceID,[CityID]=@CityID,[AreaID]=@AreaID,[Province]=@Province,[City]=@City,[Area]=@Area,[IsHidden]=@IsHidden,[IsRecommend]=@IsRecommend,[Sequence]=@Sequence,[VisitCount]=@VisitCount,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_Products",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_Products");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_Products]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_Products");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_Products");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_Products",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_Products GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_Products] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_Products Entity = new ADeeWu.HuoBi3J.Model.Shop_Products();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.ShopID = long.Parse(dr["ShopID"].ToString());
			...
			Entity.Stock = int.Parse(dr["Stock"].ToString());
			Entity.SaleOut = int.Parse(dr["SaleOut"].ToString());
			Entity.Name = (string)(dr["Name"]);
			Entity.Summary = (string)(dr["Summary"]);
			Entity.Keywords = (string)(dr["Keywords"]);
			Entity.Content = (string)(dr["Content"]);
			Entity.Price = decimal.Parse(dr["Price"].ToString());
			Entity.ReturnPrecent = int.Parse(dr["ReturnPrecent"].ToString());
			Entity.UsePriceForReturn = bool.Parse(dr["UsePriceForReturn"].ToString());
			Entity.FloatPrice = (string)(dr["FloatPrice"]);
			Entity.Picture0 = (string)(dr["Picture0"]);
			Entity.Picture1 = (string)(dr["Picture1"]);
			Entity.PictureList = (string)(dr["PictureList"]);
			Entity.SaleTime = (string)(dr["SaleTime"]);
			Entity.SaleAddress = (string)(dr["SaleAddress"]);
			Entity.HasContract = bool.Parse(dr["HasContract"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Shop_Products GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_Products[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_Products GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_Products[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_Products] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_Products[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_Products[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_Products","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Products","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Products","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_Products","ID",where,orderBy);
		}
