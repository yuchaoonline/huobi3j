...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_UserInRoles{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Admin_UserInRoles] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_UserInRoles model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@RoleID",model.RoleID.HasValue ? (object)model.RoleID.Value : (object)DBNull.Value );
		db.Parameters.Append("@AdminID",model.AdminID.HasValue ? (object)model.AdminID.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Admin_UserInRoles]([RoleID],[AdminID]) values (@RoleID,@AdminID);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_UserInRoles model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@RoleID",model.RoleID.HasValue ? (object)model.RoleID.Value : (object)DBNull.Value );
		db.Parameters.Append("@AdminID",model.AdminID.HasValue ? (object)model.AdminID.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Admin_UserInRoles] set [RoleID]=@RoleID,[AdminID]=@AdminID where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_UserInRoles",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_UserInRoles");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_UserInRoles]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_UserInRoles");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_UserInRoles");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_UserInRoles",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Admin_UserInRoles GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Admin_UserInRoles] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Admin_UserInRoles Entity = new ADeeWu.HuoBi3J.Model.Admin_UserInRoles();
		...
		public ADeeWu.HuoBi3J.Model.Admin_UserInRoles GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_UserInRoles[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Admin_UserInRoles GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Admin_UserInRoles[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Admin_UserInRoles] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Admin_UserInRoles[] EntityList = new ADeeWu.HuoBi3J.Model.Admin_UserInRoles[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Admin_UserInRoles","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserInRoles","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserInRoles","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_UserInRoles","ID",where,orderBy);
		}
