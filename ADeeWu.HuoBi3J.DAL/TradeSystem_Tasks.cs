...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class TradeSystem_Tasks{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [TradeSystem_Tasks] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.TradeSystem_Tasks model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@TaskState",model.TaskState.HasValue ? (object)model.TaskState.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [TradeSystem_Tasks]([TotalFee],[FromUserID],[ToUserID],[TaskState],[Notes],[Result],[CreateTime],[ModifyTime]) values (@TotalFee,@FromUserID,@ToUserID,@TaskState,@Notes,@Result,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.TradeSystem_Tasks model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [TradeSystem_Tasks] set [TotalFee]=@TotalFee,[FromUserID]=@FromUserID,[ToUserID]=@ToUserID,[TaskState]=@TaskState,[Notes]=@Notes,[Result]=@Result,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","TradeSystem_Tasks",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","TradeSystem_Tasks");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [TradeSystem_Tasks]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "TradeSystem_Tasks");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "TradeSystem_Tasks");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "TradeSystem_Tasks",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.TradeSystem_Tasks GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [TradeSystem_Tasks] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.TradeSystem_Tasks Entity = new ADeeWu.HuoBi3J.Model.TradeSystem_Tasks();
			...
			Entity.TaskState = dr["TaskState"] as int?;
			Entity.Notes = (string)(dr["Notes"]);
			Entity.Result = (string)(dr["Result"]);
		...
		public ADeeWu.HuoBi3J.Model.TradeSystem_Tasks GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.TradeSystem_Tasks[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.TradeSystem_Tasks GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.TradeSystem_Tasks[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [TradeSystem_Tasks] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.TradeSystem_Tasks[] EntityList = new ADeeWu.HuoBi3J.Model.TradeSystem_Tasks[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"TradeSystem_Tasks","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_Tasks","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_Tasks","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"TradeSystem_Tasks","ID",where,orderBy);
		}
