...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_QuestionIndex{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_QuestionIndex] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_QuestionIndex model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Center_QuestionIndex]([QID],[CreateTime]) values (@QID,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_QuestionIndex model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@QID",model.QID.HasValue ? (object)model.QID.Value : (object)DBNull.Value );
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Center_QuestionIndex] set [QID]=@QID,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_QuestionIndex",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_QuestionIndex");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_QuestionIndex]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_QuestionIndex");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_QuestionIndex");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_QuestionIndex",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_QuestionIndex GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_QuestionIndex] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_QuestionIndex Entity = new ADeeWu.HuoBi3J.Model.Center_QuestionIndex();
		...
		public ADeeWu.HuoBi3J.Model.Center_QuestionIndex GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_QuestionIndex[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_QuestionIndex GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_QuestionIndex[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_QuestionIndex] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_QuestionIndex[] EntityList = new ADeeWu.HuoBi3J.Model.Center_QuestionIndex[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_QuestionIndex","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_QuestionIndex","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_QuestionIndex","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_QuestionIndex","ID",where,orderBy);
		}
