...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Key_HotKey{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Key_HotKey] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Key_HotKey model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Name",model.Name );
		db.Parameters.Append("@DataType",model.DataType );
		db.Parameters.Append("@DataLink",model.DataLink );
		    DataTable dt = db.Select("insert into [Key_HotKey]([Name],[DataType],[DataLink]) values (@Name,@DataType,@DataLink);select @@Identity;");
            int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Key_HotKey model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@DataType",model.DataType );
		db.Parameters.Append("@DataLink",model.DataLink );
			return db.ExecuteSql("update [Key_HotKey] set [Name]=@Name,[DataType]=@DataType,[DataLink]=@DataLink where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Key_HotKey",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Key_HotKey");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Key_HotKey]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Key_HotKey");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Key_HotKey");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Key_HotKey",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Key_HotKey GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Key_HotKey] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Key_HotKey Entity = new ADeeWu.HuoBi3J.Model.Key_HotKey();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.Name = dr["Name"] as string;
			Entity.DataType = dr["DataType"] as string;
			Entity.DataLink = dr["DataLink"] as string;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Key_HotKey GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Key_HotKey[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Key_HotKey GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Key_HotKey[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Key_HotKey] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Key_HotKey[] EntityList = new ADeeWu.HuoBi3J.Model.Key_HotKey[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Key_HotKey","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_HotKey","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_HotKey","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Key_HotKey","ID",where,orderBy);
		}
