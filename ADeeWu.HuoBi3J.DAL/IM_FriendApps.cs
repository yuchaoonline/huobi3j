...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_FriendApps{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_FriendApps] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_FriendApps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Message",model.Message);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_FriendApps]([FromUserID],[FromUIN],[ToUserID],[ToUIN],[Message],[CreateTime]) values (@FromUserID,@FromUIN,@ToUserID,@ToUIN,@Message,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_FriendApps model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Message",model.Message);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_FriendApps] set [FromUserID]=@FromUserID,[FromUIN]=@FromUIN,[ToUserID]=@ToUserID,[ToUIN]=@ToUIN,[Message]=@Message,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_FriendApps",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_FriendApps");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_FriendApps]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_FriendApps");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_FriendApps");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_FriendApps",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_FriendApps GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_FriendApps] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_FriendApps Entity = new ADeeWu.HuoBi3J.Model.IM_FriendApps();
			...
			Entity.Message = (string)(dr["Message"]);
			Entity.CreateTime = DateTime.Parse(dr["CreateTime"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.IM_FriendApps GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_FriendApps[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_FriendApps GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_FriendApps[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_FriendApps] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_FriendApps[] EntityList = new ADeeWu.HuoBi3J.Model.IM_FriendApps[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_FriendApps","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendApps","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendApps","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_FriendApps","ID",where,orderBy);
		}
