...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_UINS{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_UINS] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_UINS model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UIN",model.UIN);
		db.Parameters.Append("@HasUsed",model.HasUsed);
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_UINS]([UIN],[HasUsed],[IsRecommend],[Sequence],[CreateTime]) values (@UIN,@HasUsed,@IsRecommend,@Sequence,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_UINS model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Sequence",model.Sequence);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_UINS] set [UIN]=@UIN,[HasUsed]=@HasUsed,[IsRecommend]=@IsRecommend,[Sequence]=@Sequence,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_UINS",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_UINS");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_UINS]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_UINS");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_UINS");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_UINS",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_UINS GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_UINS] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_UINS Entity = new ADeeWu.HuoBi3J.Model.IM_UINS();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UIN = (string)(dr["UIN"]);
			Entity.HasUsed = bool.Parse(dr["HasUsed"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.IM_UINS GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_UINS[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_UINS GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_UINS[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_UINS] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_UINS[] EntityList = new ADeeWu.HuoBi3J.Model.IM_UINS[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_UINS","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UINS","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UINS","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_UINS","ID",where,orderBy);
		}
