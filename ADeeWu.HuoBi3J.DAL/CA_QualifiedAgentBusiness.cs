...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_QualifiedAgentBusiness{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_QualifiedAgentBusiness] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@AgentUserID",model.AgentUserID);
		db.Parameters.Append("@CustomerCorpID",model.CustomerCorpID);
		db.Parameters.Append("@CustomerUserID",model.CustomerUserID);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [CA_QualifiedAgentBusiness]([AgentUserID],[CustomerCorpID],[CustomerUserID],[CreateTime],[ModifyTime]) values (@AgentUserID,@CustomerCorpID,@CustomerUserID,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime.HasValue ? (object)model.ModifyTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [CA_QualifiedAgentBusiness] set [AgentUserID]=@AgentUserID,[CustomerCorpID]=@CustomerCorpID,[CustomerUserID]=@CustomerUserID,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_QualifiedAgentBusiness",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_QualifiedAgentBusiness");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_QualifiedAgentBusiness]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_QualifiedAgentBusiness");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_QualifiedAgentBusiness");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_QualifiedAgentBusiness",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_QualifiedAgentBusiness] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness Entity = new ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.AgentUserID = long.Parse(dr["AgentUserID"].ToString());
			Entity.CustomerCorpID = long.Parse(dr["CustomerCorpID"].ToString());
			Entity.CustomerUserID = long.Parse(dr["CustomerUserID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_QualifiedAgentBusiness] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness[] EntityList = new ADeeWu.HuoBi3J.Model.CA_QualifiedAgentBusiness[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_QualifiedAgentBusiness","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QualifiedAgentBusiness","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QualifiedAgentBusiness","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_QualifiedAgentBusiness","ID",where,orderBy);
		}
