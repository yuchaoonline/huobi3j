...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class User_DealHistories{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [User_DealHistories] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.User_DealHistories model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@InMoney",model.InMoney);
		db.Parameters.Append("@OutMoney",model.OutMoney);
		db.Parameters.Append("@Balance",model.Balance);
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [User_DealHistories]([UserID],[InMoney],[OutMoney],[Balance],[Notes],[CreateTime]) values (@UserID,@InMoney,@OutMoney,@Balance,@Notes,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.User_DealHistories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [User_DealHistories] set [UserID]=@UserID,[InMoney]=@InMoney,[OutMoney]=@OutMoney,[Balance]=@Balance,[Notes]=@Notes,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","User_DealHistories",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","User_DealHistories");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [User_DealHistories]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "User_DealHistories");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "User_DealHistories");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "User_DealHistories",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.User_DealHistories GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [User_DealHistories] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.User_DealHistories Entity = new ADeeWu.HuoBi3J.Model.User_DealHistories();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = dr["UserID"] as long?;
			Entity.InMoney = decimal.Parse(dr["InMoney"].ToString());
			Entity.OutMoney = decimal.Parse(dr["OutMoney"].ToString());
			Entity.Balance = decimal.Parse(dr["Balance"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.User_DealHistories GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.User_DealHistories[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.User_DealHistories GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.User_DealHistories[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [User_DealHistories] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.User_DealHistories[] EntityList = new ADeeWu.HuoBi3J.Model.User_DealHistories[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"User_DealHistories","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_DealHistories","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_DealHistories","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_DealHistories","ID",where,orderBy);
		}
