...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Coupons_CashWhenFee_Condition{
		...
		public int Update(ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_Condition model)
		{
			db.Parameters.Clear();
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Coupons_CashWhenFee_Condition",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Coupons_CashWhenFee_Condition");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Coupons_CashWhenFee_Condition]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Coupons_CashWhenFee_Condition");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Coupons_CashWhenFee_Condition");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Coupons_CashWhenFee_Condition",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_Condition GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_Condition[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"Coupons_CashWhenFee_Condition","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_Condition","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_Condition","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_Condition","ID",where,orderBy);
		}
