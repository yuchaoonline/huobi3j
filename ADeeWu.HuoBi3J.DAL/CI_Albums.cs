...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CI_Albums{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CI_Albums] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CI_Albums model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CI_Albums]([CorpID],[FaceID],[Title],[Sequence],[IsRecommend],[IsHidden],[CreateTime],[ModifyTime]) values (@CorpID,@FaceID,@Title,@Sequence,@IsRecommend,@IsHidden,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CI_Albums model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CI_Albums] set [CorpID]=@CorpID,[FaceID]=@FaceID,[Title]=@Title,[Sequence]=@Sequence,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CI_Albums",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CI_Albums");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CI_Albums]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CI_Albums");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CI_Albums");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CI_Albums",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CI_Albums GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CI_Albums] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CI_Albums Entity = new ADeeWu.HuoBi3J.Model.CI_Albums();
		...
		public ADeeWu.HuoBi3J.Model.CI_Albums GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CI_Albums[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CI_Albums GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CI_Albums[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CI_Albums] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CI_Albums[] EntityList = new ADeeWu.HuoBi3J.Model.CI_Albums[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CI_Albums","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Albums","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Albums","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Albums","ID",where,orderBy);
		}
