...
namespace ADeeWu.HuoBi3J.DAL
{
	...
	public class Coupons_CashWhenFee_ShareFriendLog{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Coupons_CashWhenFee_ShareFriendLog] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public int Add(ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@DateTime",model.DateTime.HasValue ? (object)model.DateTime.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Coupons_CashWhenFee_ShareFriendLog]([CodeID],[UserID],[DateTime]) values (@CodeID,@UserID,@DateTime);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@DateTime",model.DateTime.HasValue ? (object)model.DateTime.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Coupons_CashWhenFee_ShareFriendLog] set [CodeID]=@CodeID,[UserID]=@UserID,[DateTime]=@DateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Coupons_CashWhenFee_ShareFriendLog",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Coupons_CashWhenFee_ShareFriendLog");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [Coupons_CashWhenFee_ShareFriendLog]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Coupons_CashWhenFee_ShareFriendLog");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Coupons_CashWhenFee_ShareFriendLog");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Coupons_CashWhenFee_ShareFriendLog",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Coupons_CashWhenFee_ShareFriendLog] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog Entity = new ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog();
		...
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Coupons_CashWhenFee_ShareFriendLog] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog[] EntityList = new ADeeWu.HuoBi3J.Model.Coupons_CashWhenFee_ShareFriendLog[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Coupons_CashWhenFee_ShareFriendLog","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_ShareFriendLog","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_ShareFriendLog","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Coupons_CashWhenFee_ShareFriendLog","ID",where,orderBy);
		}
