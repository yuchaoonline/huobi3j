...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Center_Key_Log{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Center_Key_Log] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Center_Key_Log model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Remark",model.Remark );
		    DataTable dt = db.Select("insert into [Center_Key_Log]([UID],[KID],[Price],[CreateTime],[Remark]) values (@UID,@KID,@Price,@CreateTime,@Remark);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Center_Key_Log model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime.HasValue ? (object)model.CreateTime.Value : (object)DBNull.Value );
		db.Parameters.Append("@Remark",model.Remark );
			return db.ExecuteSql("update [Center_Key_Log] set [UID]=@UID,[KID]=@KID,[Price]=@Price,[CreateTime]=@CreateTime,[Remark]=@Remark where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Center_Key_Log",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Center_Key_Log");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Center_Key_Log]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Center_Key_Log");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Center_Key_Log");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Center_Key_Log",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Log GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Center_Key_Log] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Center_Key_Log Entity = new ADeeWu.HuoBi3J.Model.Center_Key_Log();
			...
			Entity.Remark = dr["Remark"] as string;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Log GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Center_Key_Log[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Center_Key_Log GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Center_Key_Log[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Center_Key_Log] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Center_Key_Log[] EntityList = new ADeeWu.HuoBi3J.Model.Center_Key_Log[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Center_Key_Log","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Log","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Log","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Center_Key_Log","ID",where,orderBy);
		}
