...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class SD_SeekerFavs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [SD_SeekerFavs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.SD_SeekerFavs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [SD_SeekerFavs]([UserID],[SupplyDemandID],[Notes],[CreateTime],[ModifyTime]) values (@UserID,@SupplyDemandID,@Notes,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.SD_SeekerFavs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [SD_SeekerFavs] set [UserID]=@UserID,[SupplyDemandID]=@SupplyDemandID,[Notes]=@Notes,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","SD_SeekerFavs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","SD_SeekerFavs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [SD_SeekerFavs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "SD_SeekerFavs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "SD_SeekerFavs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "SD_SeekerFavs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.SD_SeekerFavs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [SD_SeekerFavs] where 1=1  and [ID]=@ID");
		...
		public ADeeWu.HuoBi3J.Model.SD_SeekerFavs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.SD_SeekerFavs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.SD_SeekerFavs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.SD_SeekerFavs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [SD_SeekerFavs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.SD_SeekerFavs[] EntityList = new ADeeWu.HuoBi3J.Model.SD_SeekerFavs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"SD_SeekerFavs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_SeekerFavs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_SeekerFavs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_SeekerFavs","ID",where,orderBy);
		}
