...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class IM_Friends{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [IM_Friends] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.IM_Friends model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@UserUIN",model.UserUIN);
		db.Parameters.Append("@FriendUserID",model.FriendUserID);
		db.Parameters.Append("@FriendUIN",model.FriendUIN);
		db.Parameters.Append("@Alias",model.Alias);
		db.Parameters.Append("@CheckState",model.CheckState);
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [IM_Friends]([UserID],[UserUIN],[FriendUserID],[FriendUIN],[Alias],[CheckState],[CreateTime]) values (@UserID,@UserUIN,@FriendUserID,@FriendUIN,@Alias,@CheckState,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.IM_Friends model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CheckState",model.CheckState);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [IM_Friends] set [UserID]=@UserID,[UserUIN]=@UserUIN,[FriendUserID]=@FriendUserID,[FriendUIN]=@FriendUIN,[Alias]=@Alias,[CheckState]=@CheckState,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","IM_Friends",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","IM_Friends");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [IM_Friends]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "IM_Friends");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "IM_Friends");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "IM_Friends",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.IM_Friends GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [IM_Friends] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.IM_Friends Entity = new ADeeWu.HuoBi3J.Model.IM_Friends();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.UserUIN = (string)(dr["UserUIN"]);
			Entity.FriendUserID = long.Parse(dr["FriendUserID"].ToString());
			Entity.FriendUIN = (string)(dr["FriendUIN"]);
			Entity.Alias = (string)(dr["Alias"]);
		...
		public ADeeWu.HuoBi3J.Model.IM_Friends GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.IM_Friends[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.IM_Friends GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.IM_Friends[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [IM_Friends] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.IM_Friends[] EntityList = new ADeeWu.HuoBi3J.Model.IM_Friends[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"IM_Friends","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Friends","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Friends","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"IM_Friends","ID",where,orderBy);
		}
