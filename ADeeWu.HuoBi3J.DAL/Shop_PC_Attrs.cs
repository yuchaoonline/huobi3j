...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Shop_PC_Attrs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Shop_PC_Attrs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Shop_PC_Attrs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CategoryID",model.CategoryID);
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@ValueType",model.ValueType);
		db.Parameters.Append("@IsValueRequire",model.IsValueRequire);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [Shop_PC_Attrs]([CategoryID],[Name],[ValueType],[IsValueRequire],[Sequence],[CreateTime],[ModifyTime]) values (@CategoryID,@Name,@ValueType,@IsValueRequire,@Sequence,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Shop_PC_Attrs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [Shop_PC_Attrs] set [CategoryID]=@CategoryID,[Name]=@Name,[ValueType]=@ValueType,[IsValueRequire]=@IsValueRequire,[Sequence]=@Sequence,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Shop_PC_Attrs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Shop_PC_Attrs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Shop_PC_Attrs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Shop_PC_Attrs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Shop_PC_Attrs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Shop_PC_Attrs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attrs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Shop_PC_Attrs] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Shop_PC_Attrs Entity = new ADeeWu.HuoBi3J.Model.Shop_PC_Attrs();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CategoryID = long.Parse(dr["CategoryID"].ToString());
			Entity.Name = (string)(dr["Name"]);
			Entity.ValueType = int.Parse(dr["ValueType"].ToString());
			Entity.IsValueRequire = bool.Parse(dr["IsValueRequire"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attrs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Shop_PC_Attrs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attrs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Shop_PC_Attrs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Shop_PC_Attrs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Shop_PC_Attrs[] EntityList = new ADeeWu.HuoBi3J.Model.Shop_PC_Attrs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Shop_PC_Attrs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attrs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attrs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Shop_PC_Attrs","ID",where,orderBy);
		}
