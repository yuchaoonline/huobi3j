...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_Roles{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_Roles] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_Roles model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ManagerCorpID",model.ManagerCorpID);
		db.Parameters.Append("@RoleName",model.RoleName);
		db.Parameters.Append("@Description",model.Description);
		    DataTable dt = db.Select("insert into [CA_Roles]([ManagerCorpID],[RoleName],[Description]) values (@ManagerCorpID,@RoleName,@Description);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_Roles model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@RoleName",model.RoleName);
		db.Parameters.Append("@Description",model.Description);
			return db.ExecuteSql("update [CA_Roles] set [ManagerCorpID]=@ManagerCorpID,[RoleName]=@RoleName,[Description]=@Description where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_Roles",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_Roles");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_Roles]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_Roles");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_Roles");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_Roles",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_Roles GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_Roles] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_Roles Entity = new ADeeWu.HuoBi3J.Model.CA_Roles();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.ManagerCorpID = long.Parse(dr["ManagerCorpID"].ToString());
			Entity.RoleName = (string)(dr["RoleName"]);
			Entity.Description = (string)(dr["Description"]);
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.CA_Roles GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_Roles[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_Roles GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_Roles[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_Roles] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_Roles[] EntityList = new ADeeWu.HuoBi3J.Model.CA_Roles[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_Roles","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_Roles","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_Roles","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_Roles","ID",where,orderBy);
		}
