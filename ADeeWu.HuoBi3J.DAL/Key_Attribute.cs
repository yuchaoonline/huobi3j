...
namespace ADeeWu.HuoBi3J.DAL
{
    ...
    public class Key_Attribute
    {
        ...
        /// 成功返回大于0的新ID
        /// </summary>
        public int Add(ADeeWu.HuoBi3J.Model.Key_Attribute model)
        {
            db.Parameters.Clear();
            ...
            db.Parameters.Append("@DataValue", model.DataValue);
            DataTable dt = db.Select("insert into [Key_Attribute]([KID],[DataType],[DataValue]) values (@KID,@DataType,@DataValue);select @@Identity;");
            int newID = int.Parse(dt.Rows[0][0].ToString());
            model.ID = newID;
            return newID;
        }

        public int Update(ADeeWu.HuoBi3J.Model.Key_Attribute model)
        {
            db.Parameters.Clear();
            ...
            db.Parameters.Append("@DataType", model.DataType);
            db.Parameters.Append("@DataValue", model.DataValue);
            return db.ExecuteSql("update [Key_Attribute] set [KID]=@KID,[DataType]=@DataType,[DataValue]=@DataValue where [ID]=@ID");
        }
        ...
        {
            string sql = String.Format("update [{0}] set {1}=@{1}", "Key_Attribute", columnName);
        ...
        public int Update(string[] updateColumns, object[] updateValues, string where)
        {
            ...
            string sql = String.Format("update [{0}] ", "Key_Attribute");
        ...
        public int Delete(int ID)
        {
            ...
            return db.ExecuteSql("delete from [Key_Attribute]  where  @ID=ID");
        }
        ...
        {
            string sql = string.Format("delete from [{0}] ", "Key_Attribute");
            return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Key_Attribute");
            if (string.IsNullOrEmpty(where))
            {
                return -1;
            }
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Key_Attribute", columnName);
            db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
        ...
        public ADeeWu.HuoBi3J.Model.Key_Attribute GetEntity(string[] columns, params object[] values)
        {
            ADeeWu.HuoBi3J.Model.Key_Attribute[] EntityList = GetEntityList("", columns, values);
        ...
        {
            return db.Select(-1, -1, "Key_Attribute", "ID", where, orderBy);
        }
        ...
        {
            return db.Select(pageSize, pageIndex, "Key_Attribute", "ID", "", "");
        }
        ...
        {
            return db.Select(pageSize, pageIndex, "Key_Attribute", "ID", "", orderBy);
        }
        ...
        {
            return db.Select(pageSize, pageIndex, "Key_Attribute", "ID", where, orderBy);
        }
