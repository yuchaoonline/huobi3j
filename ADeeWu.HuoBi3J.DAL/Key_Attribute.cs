...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Key_Attribute{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Key_Attribute model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@KID",model.KID.HasValue ? (object)model.KID.Value : (object)DBNull.Value );
        db.Parameters.Append("@KeyType", model.KeyType);
        db.Parameters.Append("@KeySize", model.KeySize);
		db.Parameters.Append("@KeyPrice",model.KeyPrice );
		    DataTable dt = db.Select("insert into [Key_Attribute]([KID],[KeyType],[KeySize],[KeyPrice]) values (@KID,@KeyType,@KeySize,@KeyPrice);select @@Identity;");
		...
		db.Parameters.Append("@KeySize",model.KeySize );
		db.Parameters.Append("@KeyPrice",model.KeyPrice );
			return db.ExecuteSql("update [Key_Attribute] set [KID]=@KID,[KeyType]=@KeyType,[KeySize]=@KeySize,[KeyPrice]=@KeyPrice where [ID]=@ID");
		}
		...
		public ADeeWu.HuoBi3J.Model.Key_Attribute GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			...
			Entity.KeyType = dr["KeyType"] as string;
			Entity.KeySize = dr["KeySize"] as string;
			Entity.KeyPrice = dr["KeyPrice"] as string;
			return Entity;
		}
