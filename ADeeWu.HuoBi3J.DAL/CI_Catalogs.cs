...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CI_Catalogs{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CI_Catalogs] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CI_Catalogs model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CorpID",model.CorpID);
		db.Parameters.Append("@FaceID",model.FaceID);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CI_Catalogs]([CorpID],[FaceID],[Title],[Sequence],[IsRecommend],[IsHidden],[CreateTime],[ModifyTime]) values (@CorpID,@FaceID,@Title,@Sequence,@IsRecommend,@IsHidden,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CI_Catalogs model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CI_Catalogs] set [CorpID]=@CorpID,[FaceID]=@FaceID,[Title]=@Title,[Sequence]=@Sequence,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CI_Catalogs",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CI_Catalogs");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CI_Catalogs]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CI_Catalogs");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CI_Catalogs");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CI_Catalogs",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CI_Catalogs GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CI_Catalogs] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CI_Catalogs Entity = new ADeeWu.HuoBi3J.Model.CI_Catalogs();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CorpID = long.Parse(dr["CorpID"].ToString());
			Entity.FaceID = long.Parse(dr["FaceID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.CI_Catalogs GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CI_Catalogs[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CI_Catalogs GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CI_Catalogs[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CI_Catalogs] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CI_Catalogs[] EntityList = new ADeeWu.HuoBi3J.Model.CI_Catalogs[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CI_Catalogs","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Catalogs","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Catalogs","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Catalogs","ID",where,orderBy);
		}
