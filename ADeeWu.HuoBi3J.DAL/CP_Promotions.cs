...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CP_Promotions{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CP_Promotions] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CP_Promotions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [CP_Promotions]([UserID],[CorpID],[Title],[Summary],[Url],[AdminUserID],[IsHidden],[CheckState],[CreateTime],[ModifyTime]) values (@UserID,@CorpID,@Title,@Summary,@Url,@AdminUserID,@IsHidden,@CheckState,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CP_Promotions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [CP_Promotions] set [UserID]=@UserID,[CorpID]=@CorpID,[Title]=@Title,[Summary]=@Summary,[Url]=@Url,[AdminUserID]=@AdminUserID,[IsHidden]=@IsHidden,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CP_Promotions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CP_Promotions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CP_Promotions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CP_Promotions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CP_Promotions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CP_Promotions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CP_Promotions GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CP_Promotions] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CP_Promotions Entity = new ADeeWu.HuoBi3J.Model.CP_Promotions();
		...
		public ADeeWu.HuoBi3J.Model.CP_Promotions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CP_Promotions[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CP_Promotions GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CP_Promotions[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CP_Promotions] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CP_Promotions[] EntityList = new ADeeWu.HuoBi3J.Model.CP_Promotions[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CP_Promotions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Promotions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Promotions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CP_Promotions","ID",where,orderBy);
		}
