...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class HotPlace{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [HotPlace] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.HotPlace model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Name",model.Name );
		    DataTable dt = db.Select("insert into [HotPlace]([Name]) values (@Name);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.HotPlace model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@Name",model.Name );
			return db.ExecuteSql("update [HotPlace] set [Name]=@Name where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","HotPlace",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","HotPlace");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [HotPlace]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "HotPlace");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "HotPlace");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "HotPlace",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.HotPlace GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [HotPlace] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.HotPlace Entity = new ADeeWu.HuoBi3J.Model.HotPlace();
		...
		public ADeeWu.HuoBi3J.Model.HotPlace GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.HotPlace[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.HotPlace GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.HotPlace[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [HotPlace] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.HotPlace[] EntityList = new ADeeWu.HuoBi3J.Model.HotPlace[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"HotPlace","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"HotPlace","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"HotPlace","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"HotPlace","ID",where,orderBy);
		}
