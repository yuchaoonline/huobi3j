...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_Page{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_Page] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_Page model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Title",model.Title );
		db.Parameters.Append("@Description",model.Description );
		db.Parameters.Append("@ParentID",model.ParentID.HasValue ? (object)model.ParentID.Value : (object)DBNull.Value );
		    DataTable dt = db.Select("insert into [Forum_Page]([Title],[Description],[ParentID]) values (@Title,@Description,@ParentID);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_Page model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Description",model.Description );
		db.Parameters.Append("@ParentID",model.ParentID.HasValue ? (object)model.ParentID.Value : (object)DBNull.Value );
			return db.ExecuteSql("update [Forum_Page] set [Title]=@Title,[Description]=@Description,[ParentID]=@ParentID where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_Page",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_Page");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_Page]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_Page");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_Page");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_Page",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_Page GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_Page] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_Page Entity = new ADeeWu.HuoBi3J.Model.Forum_Page();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.Title = dr["Title"] as string;
			Entity.Description = dr["Description"] as string;
			Entity.ParentID = dr["ParentID"] as long?;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Forum_Page GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_Page[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_Page GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_Page[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_Page] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_Page[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_Page[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_Page","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Page","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Page","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Page","ID",where,orderBy);
		}
