...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class FreeAdmission{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [FreeAdmission] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.FreeAdmission model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@SeqNo",model.SeqNo );
		db.Parameters.Append("@SeqPwd",model.SeqPwd );
		db.Parameters.Append("@StartDate",model.StartDate);
		db.Parameters.Append("@EndDate",model.EndDate);
		db.Parameters.Append("@Money",model.Money);
		db.Parameters.Append("@TotalCount",model.TotalCount);
		db.Parameters.Append("@ApplyCount",model.ApplyCount);
		    DataTable dt = db.Select("insert into [FreeAdmission]([SeqNo],[SeqPwd],[StartDate],[EndDate],[Money],[TotalCount],[ApplyCount]) values (@SeqNo,@SeqPwd,@StartDate,@EndDate,@Money,@TotalCount,@ApplyCount);select @@Identity;");
			int newID = int.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.FreeAdmission model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@TotalCount",model.TotalCount);
		db.Parameters.Append("@ApplyCount",model.ApplyCount);
			return db.ExecuteSql("update [FreeAdmission] set [SeqNo]=@SeqNo,[SeqPwd]=@SeqPwd,[StartDate]=@StartDate,[EndDate]=@EndDate,[Money]=@Money,[TotalCount]=@TotalCount,[ApplyCount]=@ApplyCount where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","FreeAdmission",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","FreeAdmission");
		...
		public int Delete(int  ID)
		{
			...
			return db.ExecuteSql("delete from [FreeAdmission]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "FreeAdmission");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "FreeAdmission");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "FreeAdmission",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission GetEntity(int  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [FreeAdmission] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.FreeAdmission Entity = new ADeeWu.HuoBi3J.Model.FreeAdmission();
			Entity.ID = int.Parse(dr["ID"].ToString());
			Entity.SeqNo = dr["SeqNo"] as string;
			Entity.SeqPwd = dr["SeqPwd"] as string;
			Entity.StartDate = DateTime.Parse(dr["StartDate"].ToString());
			Entity.EndDate = DateTime.Parse(dr["EndDate"].ToString());
			Entity.Money = decimal.Parse(dr["Money"].ToString());
			Entity.TotalCount = int.Parse(dr["TotalCount"].ToString());
			Entity.ApplyCount = int.Parse(dr["ApplyCount"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.FreeAdmission[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.FreeAdmission GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.FreeAdmission[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [FreeAdmission] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.FreeAdmission[] EntityList = new ADeeWu.HuoBi3J.Model.FreeAdmission[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"FreeAdmission","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"FreeAdmission","ID",where,orderBy);
		}
