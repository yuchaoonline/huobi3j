...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class SD_Bidders{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [SD_Bidders] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.SD_Bidders model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@SupplyDemandID",model.SupplyDemandID);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [SD_Bidders]([UserID],[SupplyDemandID],[Status],[Content],[VisitCount],[CreateTime],[ModifyTime]) values (@UserID,@SupplyDemandID,@Status,@Content,@VisitCount,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.SD_Bidders model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [SD_Bidders] set [UserID]=@UserID,[SupplyDemandID]=@SupplyDemandID,[Status]=@Status,[Content]=@Content,[VisitCount]=@VisitCount,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","SD_Bidders",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","SD_Bidders");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [SD_Bidders]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "SD_Bidders");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "SD_Bidders");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "SD_Bidders",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.SD_Bidders GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [SD_Bidders] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.SD_Bidders Entity = new ADeeWu.HuoBi3J.Model.SD_Bidders();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.SupplyDemandID = long.Parse(dr["SupplyDemandID"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.SD_Bidders GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.SD_Bidders[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.SD_Bidders GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.SD_Bidders[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [SD_Bidders] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.SD_Bidders[] EntityList = new ADeeWu.HuoBi3J.Model.SD_Bidders[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"SD_Bidders","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_Bidders","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_Bidders","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"SD_Bidders","ID",where,orderBy);
		}
