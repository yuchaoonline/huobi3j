...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_Roles{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Admin_Roles] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_Roles model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@RoleName",model.RoleName);
		db.Parameters.Append("@Description",model.Description);
		    DataTable dt = db.Select("insert into [Admin_Roles]([RoleName],[Description]) values (@RoleName,@Description);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_Roles model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",model.ID);
		db.Parameters.Append("@RoleName",model.RoleName);
		db.Parameters.Append("@Description",model.Description);
			return db.ExecuteSql("update [Admin_Roles] set [RoleName]=@RoleName,[Description]=@Description where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_Roles",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_Roles");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_Roles]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_Roles");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_Roles");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_Roles",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Admin_Roles GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Admin_Roles] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Admin_Roles Entity = new ADeeWu.HuoBi3J.Model.Admin_Roles();
		...
		public ADeeWu.HuoBi3J.Model.Admin_Roles GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_Roles[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Admin_Roles GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Admin_Roles[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Admin_Roles] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Admin_Roles[] EntityList = new ADeeWu.HuoBi3J.Model.Admin_Roles[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Admin_Roles","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Roles","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Roles","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Roles","ID",where,orderBy);
		}
