...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CA_UserPermissions{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CA_UserPermissions] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CA_UserPermissions model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@CorpAgentID",model.CorpAgentID.HasValue ? (object)model.CorpAgentID.Value : (object)DBNull.Value );
		...
		db.Parameters.Append("@AuthorizeState",model.AuthorizeState);
		db.Parameters.Append("@ManagerCorpID",model.ManagerCorpID);
		    DataTable dt = db.Select("insert into [CA_UserPermissions]([CorpAgentID],[PageID],[UserID],[AuthorizeState],[ManagerCorpID]) values (@CorpAgentID,@PageID,@UserID,@AuthorizeState,@ManagerCorpID);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CA_UserPermissions model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@AuthorizeState",model.AuthorizeState);
		db.Parameters.Append("@ManagerCorpID",model.ManagerCorpID);
			return db.ExecuteSql("update [CA_UserPermissions] set [CorpAgentID]=@CorpAgentID,[PageID]=@PageID,[UserID]=@UserID,[AuthorizeState]=@AuthorizeState,[ManagerCorpID]=@ManagerCorpID where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CA_UserPermissions",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CA_UserPermissions");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CA_UserPermissions]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CA_UserPermissions");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CA_UserPermissions");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CA_UserPermissions",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CA_UserPermissions GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CA_UserPermissions] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CA_UserPermissions Entity = new ADeeWu.HuoBi3J.Model.CA_UserPermissions();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.CorpAgentID = dr["CorpAgentID"] as long?;
			Entity.PageID = dr["PageID"] as long?;
			Entity.UserID = long.Parse(dr["UserID"].ToString());
			Entity.AuthorizeState = int.Parse(dr["AuthorizeState"].ToString());
			Entity.ManagerCorpID = long.Parse(dr["ManagerCorpID"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.CA_UserPermissions GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CA_UserPermissions[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CA_UserPermissions GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CA_UserPermissions[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CA_UserPermissions] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CA_UserPermissions[] EntityList = new ADeeWu.HuoBi3J.Model.CA_UserPermissions[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CA_UserPermissions","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_UserPermissions","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_UserPermissions","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CA_UserPermissions","ID",where,orderBy);
		}
