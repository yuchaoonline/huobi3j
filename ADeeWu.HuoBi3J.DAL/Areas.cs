...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Areas{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Areas] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Areas model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@Name",model.Name);
		db.Parameters.Append("@CID",model.CID);
		db.Parameters.Append("@Sort",model.Sort);
		    DataTable dt = db.Select("insert into [Areas]([Name],[CID],[Sort]) values (@Name,@CID,@Sort);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Areas model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CID",model.CID);
		db.Parameters.Append("@Sort",model.Sort);
			return db.ExecuteSql("update [Areas] set [Name]=@Name,[CID]=@CID,[Sort]=@Sort where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Areas",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Areas");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Areas]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Areas");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Areas");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Areas",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Areas GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Areas] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Areas Entity = new ADeeWu.HuoBi3J.Model.Areas();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.Name = (string)(dr["Name"]);
			Entity.CID = long.Parse(dr["CID"].ToString());
			Entity.Sort = int.Parse(dr["Sort"].ToString());
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Areas GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Areas[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Areas GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Areas[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Areas] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Areas[] EntityList = new ADeeWu.HuoBi3J.Model.Areas[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Areas","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Areas","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Areas","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Areas","ID",where,orderBy);
		}
