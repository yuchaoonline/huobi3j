...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class CI_Videos{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [CI_Videos] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.CI_Videos model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@PreviewURL",model.PreviewURL);
		    DataTable dt = db.Select("insert into [CI_Videos]([CorpID],[Title],[Summary],[Sequence],[IsRecommend],[IsHidden],[VisitCount],[CheckState],[CreateTime],[ModifyTime],[URL],[PreviewURL]) values (@CorpID,@Title,@Summary,@Sequence,@IsRecommend,@IsHidden,@VisitCount,@CheckState,@CreateTime,@ModifyTime,@URL,@PreviewURL);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.CI_Videos model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@URL",model.URL);
		db.Parameters.Append("@PreviewURL",model.PreviewURL);
			return db.ExecuteSql("update [CI_Videos] set [CorpID]=@CorpID,[Title]=@Title,[Summary]=@Summary,[Sequence]=@Sequence,[IsRecommend]=@IsRecommend,[IsHidden]=@IsHidden,[VisitCount]=@VisitCount,[CheckState]=@CheckState,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime,[URL]=@URL,[PreviewURL]=@PreviewURL where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","CI_Videos",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","CI_Videos");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [CI_Videos]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "CI_Videos");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "CI_Videos");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "CI_Videos",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.CI_Videos GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [CI_Videos] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.CI_Videos Entity = new ADeeWu.HuoBi3J.Model.CI_Videos();
			...
			Entity.PreviewURL = (string)(dr["PreviewURL"]);
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.CI_Videos GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.CI_Videos[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.CI_Videos GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.CI_Videos[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [CI_Videos] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.CI_Videos[] EntityList = new ADeeWu.HuoBi3J.Model.CI_Videos[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"CI_Videos","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Videos","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Videos","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"CI_Videos","ID",where,orderBy);
		}
