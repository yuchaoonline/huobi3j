...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class User_TransferApplications{
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.User_TransferApplications model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID);
		db.Parameters.Append("@TransferMoney",model.TransferMoney);
		...
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
		    DataTable dt = db.Select("insert into [User_TransferApplications]([UserID],[TransferMoney],[AlipayAccount],[CheckState],[Notes],[CreateTime],[ModifyTime]) values (@UserID,@TransferMoney,@AlipayAccount,@CheckState,@Notes,@CreateTime,@ModifyTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.User_TransferApplications model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		db.Parameters.Append("@ModifyTime",model.ModifyTime);
			return db.ExecuteSql("update [User_TransferApplications] set [UserID]=@UserID,[TransferMoney]=@TransferMoney,[AlipayAccount]=@AlipayAccount,[CheckState]=@CheckState,[Notes]=@Notes,[CreateTime]=@CreateTime,[ModifyTime]=@ModifyTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","User_TransferApplications",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","User_TransferApplications");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [User_TransferApplications]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "User_TransferApplications");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "User_TransferApplications");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "User_TransferApplications",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		...
		public ADeeWu.HuoBi3J.Model.User_TransferApplications GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.User_TransferApplications[] EntityList = GetEntityList("",columns,values);
		...
		{
			return db.Select(-1,-1,"User_TransferApplications","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_TransferApplications","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_TransferApplications","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_TransferApplications","ID",where,orderBy);
		}
