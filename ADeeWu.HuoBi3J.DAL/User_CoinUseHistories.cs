...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class User_CoinUseHistories{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [User_CoinUseHistories] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.User_CoinUseHistories model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@UserID",model.UserID.HasValue ? (object)model.UserID.Value : (object)DBNull.Value );
		db.Parameters.Append("@InCoin",model.InCoin);
		db.Parameters.Append("@OutCoin",model.OutCoin);
		...
		db.Parameters.Append("@CreateTime",model.CreateTime);
		    DataTable dt = db.Select("insert into [User_CoinUseHistories]([UserID],[InCoin],[OutCoin],[Remain],[Notes],[CreateTime]) values (@UserID,@InCoin,@OutCoin,@Remain,@Notes,@CreateTime);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.User_CoinUseHistories model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@Notes",model.Notes);
		db.Parameters.Append("@CreateTime",model.CreateTime);
			return db.ExecuteSql("update [User_CoinUseHistories] set [UserID]=@UserID,[InCoin]=@InCoin,[OutCoin]=@OutCoin,[Remain]=@Remain,[Notes]=@Notes,[CreateTime]=@CreateTime where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","User_CoinUseHistories",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","User_CoinUseHistories");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [User_CoinUseHistories]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "User_CoinUseHistories");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "User_CoinUseHistories");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "User_CoinUseHistories",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.User_CoinUseHistories GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [User_CoinUseHistories] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.User_CoinUseHistories Entity = new ADeeWu.HuoBi3J.Model.User_CoinUseHistories();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = dr["UserID"] as long?;
			Entity.InCoin = decimal.Parse(dr["InCoin"].ToString());
			Entity.OutCoin = decimal.Parse(dr["OutCoin"].ToString());
			Entity.Remain = decimal.Parse(dr["Remain"].ToString());
		...
		public ADeeWu.HuoBi3J.Model.User_CoinUseHistories GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.User_CoinUseHistories[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.User_CoinUseHistories GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.User_CoinUseHistories[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [User_CoinUseHistories] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.User_CoinUseHistories[] EntityList = new ADeeWu.HuoBi3J.Model.User_CoinUseHistories[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"User_CoinUseHistories","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_CoinUseHistories","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_CoinUseHistories","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"User_CoinUseHistories","ID",where,orderBy);
		}
