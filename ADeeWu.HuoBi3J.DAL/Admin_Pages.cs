...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Admin_Pages{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Admin_Pages] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Admin_Pages model)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@PageCode",model.PageCode);
		db.Parameters.Append("@PageName",model.PageName);
		db.Parameters.Append("@URL",model.URL);
		db.Parameters.Append("@Description",model.Description);
		    DataTable dt = db.Select("insert into [Admin_Pages]([PageCode],[PageName],[URL],[Description]) values (@PageCode,@PageName,@URL,@Description);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Admin_Pages model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@URL",model.URL);
		db.Parameters.Append("@Description",model.Description);
			return db.ExecuteSql("update [Admin_Pages] set [PageCode]=@PageCode,[PageName]=@PageName,[URL]=@URL,[Description]=@Description where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Admin_Pages",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Admin_Pages");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Admin_Pages]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Admin_Pages");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Admin_Pages");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Admin_Pages",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Admin_Pages GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Admin_Pages] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Admin_Pages Entity = new ADeeWu.HuoBi3J.Model.Admin_Pages();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.PageCode = (string)(dr["PageCode"]);
			Entity.PageName = (string)(dr["PageName"]);
		...
		public ADeeWu.HuoBi3J.Model.Admin_Pages GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Admin_Pages[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Admin_Pages GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Admin_Pages[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Admin_Pages] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Admin_Pages[] EntityList = new ADeeWu.HuoBi3J.Model.Admin_Pages[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Admin_Pages","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Pages","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Pages","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Admin_Pages","ID",where,orderBy);
		}
