...
namespace ADeeWu.HuoBi3J.DAL{
	...
	public class Forum_Users{
		...
		public bool Exist(string where)
        {
           ...
           builder.Append("select * from [Forum_Users] where 1=1");
		...
		/// 成功返回大于0的新ID
		/// </summary>
		public long Add(ADeeWu.HuoBi3J.Model.Forum_Users model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IsValid",model.IsValid.HasValue ? (object)model.IsValid.Value : (object)DBNull.Value );
		db.Parameters.Append("@Mobile",model.Mobile );
		    DataTable dt = db.Select("insert into [Forum_Users]([UserID],[CreateTime],[IsValid],[Mobile]) values (@UserID,@CreateTime,@IsValid,@Mobile);select @@Identity;");
			long newID = long.Parse(dt.Rows[0][0].ToString());
			model.ID = newID;
			return newID;
		}
		
		public int Update(ADeeWu.HuoBi3J.Model.Forum_Users model)
		{
			db.Parameters.Clear();
		...
		db.Parameters.Append("@IsValid",model.IsValid.HasValue ? (object)model.IsValid.Value : (object)DBNull.Value );
		db.Parameters.Append("@Mobile",model.Mobile);
			return db.ExecuteSql("update [Forum_Users] set [UserID]=@UserID,[CreateTime]=@CreateTime,[IsValid]=@IsValid,[Mobile]=@Mobile where [ID]=@ID");
		}
		...
		{
			string sql = String.Format("update [{0}] set {1}=@{1}","Forum_Users",columnName);
		...
		public int Update(string[] updateColumns, object[] updateValues, string where)
		{
			...
			string sql = String.Format("update [{0}] ","Forum_Users");
		...
		public int Delete(long  ID)
		{
			...
			return db.ExecuteSql("delete from [Forum_Users]  where  @ID=ID");
		}
		...
		{
			string sql = string.Format("delete from [{0}] ", "Forum_Users");
			return db.ExecuteSql(sql);
        ...
        {
            string sql = string.Format("delete from [{0}] where 1=2 ", "Forum_Users");
			if(string.IsNullOrEmpty(where)){
				return -1;
			}
        ...
        {
            string sql = string.Format("delete from [{0}] where {1}=@{1} ", "Forum_Users",columnName);
			db.Parameters.Clear();
            db.Parameters.Append("@" + columnName, value);
            return db.ExecuteSql(sql);
        }
		
		public ADeeWu.HuoBi3J.Model.Forum_Users GetEntity(long  ID)
		{
			db.Parameters.Clear();
		db.Parameters.Append("@ID",ID);
			DataTable dt = db.Select("select * from [Forum_Users] where 1=1  and [ID]=@ID");
			if(dt.Rows.Count==0) return null;
			DataRow dr = dt.Rows[0];
			ADeeWu.HuoBi3J.Model.Forum_Users Entity = new ADeeWu.HuoBi3J.Model.Forum_Users();
			Entity.ID = long.Parse(dr["ID"].ToString());
			Entity.UserID = dr["UserID"] as long?;
			Entity.CreateTime = dr["CreateTime"] as DateTime?;
			Entity.IsValid = dr["IsValid"] as bool?;
			Entity.Mobile = dr["Mobile"] as string;
			return Entity;
		}
		
		
		public ADeeWu.HuoBi3J.Model.Forum_Users GetEntity(string[] columns,params object[] values)
		{
			ADeeWu.HuoBi3J.Model.Forum_Users[] EntityList = GetEntityList("",columns,values);
			if(EntityList.Length==0)return null;
			return EntityList[0];
		}
		
		public ADeeWu.HuoBi3J.Model.Forum_Users GetEntity(string where)
		{
		...
		public ADeeWu.HuoBi3J.Model.Forum_Users[] GetEntityList(string orderBy,string[] columns,params object[] values)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select * from [Forum_Users] where 1=1");
			db.Parameters.Clear();
            ...
            ADeeWu.HuoBi3J.Model.Forum_Users[] EntityList = new ADeeWu.HuoBi3J.Model.Forum_Users[dt.Rows.Count];
		...
		{
			return db.Select(-1,-1,"Forum_Users","ID",where,orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Users","ID","","");
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Users","ID","",orderBy);
		}
		...
		{
			return db.Select(pageSize,pageIndex,"Forum_Users","ID",where,orderBy);
		}
